...
- Search the client bundle for a leaked secret key. The word `service_role` only catches a reference; the key itself is either a legacy JWT (`eyJ...`) or a new secret key (`sb_secret_...`), neither of which contains that word, so search for all three plus any private key, for example `grep -rE 'service_role|eyJ|sb_secret_' dist/`. The result must be empty; a hit is a key to rotate, not just to delete from the tree.
