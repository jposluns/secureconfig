...
since a worker with neither `capacity_per_instance` nor `max_use_count` set persists and reuses across
jobs by default, so their absence is the finding and you confirm they are present and set to `1`.
