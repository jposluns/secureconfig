...
- **Weaviate**'s REST listener depends on `--scheme`, whose default comes from its API spec and is `https`. With `--scheme http`, as the official `semitechnologies/weaviate` image passes (with `--host 0.0.0.0 --port 8080`), it takes `--host` (variable `HOST`; the default `localhost` applies only while `HOST` is unset, since an exported but empty `HOST` gives an empty host, which binds every local address) and `--port` (variable `PORT`, a random port when neither is set). With `https`, including the default, it binds `--tls-host` (variable `TLS_HOST`, falling back to the `--host` value when empty) on `--tls-port` (variable `TLS_PORT`, a random port when neither is set); `unix` binds `--socket-path`. None of Weaviate's other listeners follows `--host`. Its gRPC listener always starts and listens on port `GRPC_PORT` (default 50051) on every local address. The debug HTTP listener (pprof and fgprof) binds every local address on `GO_PROFILING_PORT` (default 6060) unless `GO_PROFILING_DISABLE` is set to a true value, and answers 404 until debug endpoints are enabled by configuration. The Prometheus listener (`/metrics` and `/tenant-activity`) binds every local address when monitoring is enabled: on 2112 when `PROMETHEUS_MONITORING_ENABLED` enables it, unless `PROMETHEUS_MONITORING_PORT` sets another port. The cluster listeners open even on a single node: memberlist gossip on `CLUSTER_GOSSIP_BIND_PORT` (default 7946, TCP and UDP) at `CLUSTER_BIND_ADDR`, or `0.0.0.0` when that is unset; the internal cluster API on `CLUSTER_DATA_BIND_PORT` (default the gossip port plus one, 7947) on every local address; and Raft on `RAFT_PORT` (default 8300) with its internal RPC on `RAFT_INTERNAL_RPC_PORT` (default 8301), at `CLUSTER_BIND_ADDR`, else `CLUSTER_ADVERTISE_ADDR`, else the private address memberlist advertises. Keep 50051, 6060, 2112, 7946, 7947, 8300 and 8301 unpublished, or open them only to cluster peers.
