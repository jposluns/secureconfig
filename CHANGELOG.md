...
## 2026-09-14
- Added tools/check_guard_conventions.py (#67), a deterministic offline gate for the curl probe conventions: a curl in a fenced shell block must lead with -q (or the reader's ~/.curlrc alters the probe), and a URL carrying a literal [ or { must carry -g. Registered in run_all_checks.sh with 60 self-test cases. The plan's blanket -g premise was falsified at re-verify (the corpus is not uniform on -g), so -g was narrowed to glob URLs and convention 2 was demoted (--no-c2, tracked by row 3.12), per the maintainer's decision.
