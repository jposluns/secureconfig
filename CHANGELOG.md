...
## 2026-09-14
- Added a security policy in SECURITY.md (#101, row 4.8): the repository had none. It routes reports to GitHub private vulnerability reporting, states the scope (guides, the plugin, and helper scripts, with documentation accuracy explicitly in scope), and notes that main is the published state with no separate release artifact. SECURITY.md is added to the gate suite's not_a_guide exclusions so a meta file is not held to guide shape. Found in an external pre-launch review.
