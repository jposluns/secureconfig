...
## 2026-09-14
- Required ME_CONFIG_BASICAUTH_ENABLED for mongo-express in admin-uis.md (#94, row 1.60): the guide set ME_CONFIG_BASICAUTH_USERNAME and ME_CONFIG_BASICAUTH_PASSWORD, but in mongo-express 1.x basic auth is off by default (ME_CONFIG_BASICAUTH_ENABLED defaults to false) and credentials alone do not enable it, so a reader following the guide ran the panel with no login. The block now sets ME_CONFIG_BASICAUTH_ENABLED=true and the intro states the default. Verified at the mongo-express README. Found in an external pre-launch review.
