...
sshd uses the *first* value it reads for each option, and it includes `/etc/ssh/sshd_config.d/*.conf` at the very top of the main file. Cloud images ship a `50-cloud-init.conf` in that directory that sets `PasswordAuthentication yes`, so the same option placed later, in the main file or a higher-numbered drop-in, is silently overridden and password login stays on. A `00-` file is read before it.
...
sudo sshd -T | grep -iE 'passwordauthentication|permitrootlogin|kbdinteractiveauthentication'
                                             # the EFFECTIVE values after every include; confirm they
                                             # match what you set, not a cloud-init drop-in read earlier
```
