...
Protocol-level sessions and `Mcp-Session-Id` are removed, as are the standalone GET stream and stream resumability via `Last-Event-ID`. For older-client traffic, a server supporting only 2026-07-28 SHOULD ignore those removed legacy headers and return `405` for GET/DELETE requests. See the [Changelog][mcp-changelog] and [Streamable HTTP][mcp-http]. Readers maintaining older deployments can consult the explicitly **legacy** [2025-11-25 transport reference][mcp-legacy], a handshake-based Final revision.

## 1. Prefer stdio, and keep HTTP on loopback

- For a single-user tool on a workstation, use the stdio transport. The spec's own guidance for local servers is to "use the `stdio` transport to limit access to just the MCP client" and, if HTTP is used anyway, to require an authorization token or use a Unix domain socket with restricted access. See [Security Best Practices][mcp-security].
- Give a stdio server only the environment it needs: the API key for the one service it wraps, not a shell profile full of cloud credentials. Per the spec, stdio servers "retrieve credentials from the environment", which means that environment is the server's whole privilege set ([secrets.md](secrets.md)). See [Authorization][mcp-auth].
- For Streamable HTTP, the spec's Security Warning says: "When running locally, servers SHOULD bind only to localhost (127.0.0.1) rather than all network interfaces (0.0.0.0)". Bind to `127.0.0.1` and publish only a fronting layer, exactly as [ollama.md](ollama.md) does for a model server. Check the SDK or framework you use for its bind-address option; do not assume its default is loopback. See [Streamable HTTP][mcp-http].

## 2. Validate `Origin` (DNS rebinding)

The spec requires it: "Servers MUST validate the `Origin` header on all incoming connections to prevent DNS rebinding attacks. If the `Origin` header is present and invalid, servers MUST respond with HTTP 403 Forbidden." The response body MAY be a JSON-RPC error with no `id`. Without this, "attackers could use DNS rebinding to interact with local MCP servers from remote websites": a page in the user's browser resolves its own hostname to `127.0.0.1` and drives the local server. See [Streamable HTTP][mcp-http]. Do the check in the server (most SDKs have an allowed-origins option; confirm yours is on). A reverse proxy can add a second check in front, using the nginx [`if` directive][nginx-rewrite] with the [`$http_origin` variable][nginx-core]:
...
See the [nginx map module][nginx-map] for the mapping syntax. These proxy checks validate Origin only; the MCP server still performs the required mirror-header/body validation and returns `-32020` on a mismatch. A fronting proxy does not substitute for that validation. See [Streamable HTTP, Server Validation][mcp-http].
...
**Option A, spec authorization.** The MCP server acts as an OAuth 2.1 resource server. See [Authorization][mcp-auth]. The spec's requirements, in its own terms:

- MCP servers MUST implement OAuth 2.0 Protected Resource Metadata (RFC 9728), and the metadata MUST include `authorization_servers` with at least one authorization server. Servers MUST implement at least one discovery mechanism: a `WWW-Authenticate` header carrying `resource_metadata` on `401 Unauthorized` responses, or a well-known URI. Clients MUST support both, following `resource_metadata` when present and otherwise trying the endpoint-path-suffixed well-known URI first, then the root `/.well-known/oauth-protected-resource`. See [Authorization Server Discovery][mcp-auth-discovery].
- Clients "MUST implement PKCE", MUST use `S256` "when technically capable", and "MUST refuse to proceed" if the authorization server's metadata lacks `code_challenge_methods_supported`. See [Authorization Security Considerations][mcp-auth-security]. Clients MUST send the RFC 8707 `resource` parameter in both authorization and token requests, naming the MCP server's canonical URI. See [Authorization][mcp-auth].
- Servers "MUST validate that access tokens were issued specifically for them as the intended audience"; invalid or expired tokens get `401`, insufficient scope gets `403`. Servers "MUST NOT accept or transit any other tokens": the token the client presents never travels on to an upstream API. See [Authorization][mcp-auth]. If the server calls upstream APIs, "the access token used at the upstream API is a separate token, issued by the upstream authorization server". See [Authorization Security Considerations][mcp-auth-security].
- Tokens go in `Authorization: Bearer ...` on every request, never in the URL. See [Authorization][mcp-auth].
- MCP 2026-07-28 is stateless: protocol sessions and `Mcp-Session-Id` are removed. See the [Changelog][mcp-changelog]. Servers needing cross-call state mint explicit handles. Servers MUST NOT treat possession of a state handle as authentication; servers implementing authorization MUST verify every inbound request. They SHOULD use secure, non-deterministic handles and bind them server-side to the authenticated user, for example by keying state as `<user_id>:<handle>` with the user ID derived from the verified token. See [Security Best Practices, State Handle Hijacking][mcp-security].
- **Scope hierarchy:** servers "MUST account for scope hierarchies, where a broader scope implies narrower ones, when deciding whether a token is sufficient" for an operation. Apply that rule in server-side token acceptance. See [Authorization, Step-Up Authorization Flow][mcp-auth].
- **Refresh tokens:** for public clients, the authorization server MUST rotate refresh tokens. This requirement already existed in 2025-11-25; it is not new in 2026-07-28. Treat it as an identity-provider selection criterion. The current requirement is in [Authorization Security Considerations, Token Theft][mcp-auth-security]. The MCP server SHOULD NOT advertise `offline_access` in `WWW-Authenticate` scopes or Protected Resource Metadata `scopes_supported`, because refresh tokens are not a resource requirement. See [Authorization, Refresh Tokens][mcp-auth].

Pick an identity provider from [identity-providers.md](identity-providers.md) as the authorization server; the client-side hygiene in [oidc-integration.md](oidc-integration.md) applies. MCP 2026-07-28 deprecates Dynamic Client Registration in favour of Client ID Metadata Documents (HTTPS-URL client IDs); DCR remains available for compatibility, and clients using it MUST send an appropriate `application_type`. Check which mechanisms the identity provider supports. See [Client Registration][mcp-registration].

Azure App Service can serve the RFC 9728 document for an app behind its built-in authentication: set the `WEBSITE_AUTH_PRM_DEFAULT_WITH_SCOPES` application setting to a comma-separated list of the required scopes, and the 401 challenge then carries the metadata URL and scopes. Microsoft marks this **preview** at the time of writing and says the configuration may change ([cloud-identity-proxies.md](cloud-identity-proxies.md) covers the rest of Easy Auth). See [Azure App Service authentication][azure-auth].
...
- Never forward the client's token upstream (spec: "Token passthrough" is an anti-pattern and "explicitly forbidden"). Log tool calls with the authenticated identity so an incident can be traced. See [Security Best Practices][mcp-security].

## Verify

The direct-origin request below checks listener reachability, not MCP RPC success: any HTTP response, including `405`, proves that the port answered. Use curl 7.75.0 or newer for the `exitcode` and `errormsg` write-out variables. See the [curl manual][curl-manual].
...
**Probe 2 - REASONED, not demonstrated.** No live MCP 2026-07-28 server and valid deployment credential are available in the authoring environment. Establish the allowed-Origin positive control before interpreting the negative request. Require a real `DiscoverResult`, including `supportedVersions`, `capabilities`, and the `ttlMs`/`cacheScope` caching hints required for complete discovery results. See [Discovery][mcp-discover] and [Caching][mcp-caching]. Fixed: changing only Origin produces `403`; exposed: the disallowed Origin still receives a discovery result. A `403` counts as an Origin rejection only after the otherwise-identical positive control succeeds. See [Streamable HTTP, Security & Endpoint][mcp-http]. Backlog row 1.75 tracks demonstration.

Header files keep credential values out of curl's argv; this does not protect a secret copied into shell history, exposed through tracing, or readable by the account owner. See [secrets.md](secrets.md) and the [curl manual][curl-manual].
...
# result - supportedVersions, capabilities, and the ttlMs/cacheScope caching hints - not merely a 2xx
...
disc='{"jsonrpc":"2.0","id":1,"method":"server/discover","params":{"_meta":{"io.modelcontextprotocol/protocolVersion":"2026-07-28","io.modelcontextprotocol/clientInfo":{"name":"check","version":"1.0.0"},"io.modelcontextprotocol/clientCapabilities":{}}}}'
curl -q -g -si --noproxy '*' -X POST https://mcp.example.com/mcp -H 'Origin: https://mcp.example.com' \
  -H "@$1" \
  -H 'MCP-Protocol-Version: 2026-07-28' -H 'Mcp-Method: server/discover' \
  -H 'Content-Type: application/json' -H 'Accept: application/json, text/event-stream' -d "$disc"
...
# Exposed: any 2xx, meaning the server accepted a token minted for another audience, which the spec's
# audience-validation MUST forbids. A 400 -32020 is a header mismatch, not an audience verdict.
...
(
set -- REPLACE_WITH_WRONG_AUDIENCE_HEADER_FILE
case "$1" in ""|*REPLACE_WITH_*) echo 'substitute the mode-0600 wrong-audience header file path on the set -- line above; not probing'; exit 2 ;; esac
[ -r "$1" ] || { echo 'header file not readable; not probing'; exit 2; }
disc='{"jsonrpc":"2.0","id":1,"method":"server/discover","params":{"_meta":{"io.modelcontextprotocol/protocolVersion":"2026-07-28","io.modelcontextprotocol/clientInfo":{"name":"check","version":"1.0.0"},"io.modelcontextprotocol/clientCapabilities":{}}}}'
curl -q -g -si --noproxy '*' -X POST https://mcp.example.com/mcp -H 'Origin: https://mcp.example.com' \
  -H "@$1" \
  -H 'MCP-Protocol-Version: 2026-07-28' -H 'Mcp-Method: server/discover' \
...
- Assuming the MCP endpoint still answers `GET` or carries an `Mcp-Session-Id`: under 2026-07-28 the endpoint is POST-only and protocol sessions are removed; a server supporting only this revision SHOULD return `405` for GET/DELETE. See [Streamable HTTP][mcp-http].
- Treating possession of a state handle as authentication: the spec forbids it; verify every inbound request. See [Security Best Practices][mcp-security].

## Sources (checked September 2026)

- MCP specification 2026-07-28, [Transports, Streamable HTTP][mcp-http] (POST endpoint, Origin validation, loopback binding, request headers, header/body validation, removed legacy mechanisms).
- MCP specification 2026-07-28, [Base protocol][mcp-base] (per-request `_meta`, protocol version, client capabilities and identity).
- MCP specification 2026-07-28, [Discovery, `server/discover`][mcp-discover] (server implementation requirement, optional client call, request and response shape).
- MCP specification 2026-07-28, [Changelog][mcp-changelog] (handshake and session removal, subscription streams, error-code renumbering).
- MCP specification 2026-07-28, [Authorization][mcp-auth] (OPTIONAL, OAuth resource server, credentials, `resource`, audience validation, error codes, scope hierarchy, `offline_access`).
- MCP specification 2026-07-28, [Authorization Server Discovery][mcp-auth-discovery] (RFC 9728 metadata, `authorization_servers`, challenge and well-known discovery).
- MCP specification 2026-07-28, [Authorization Security Considerations][mcp-auth-security] (PKCE, refresh-token rotation, HTTPS, separate upstream tokens).
- MCP specification 2026-07-28, [Client Registration][mcp-registration] (DCR deprecation, Client ID Metadata Documents, `application_type`).
- MCP specification 2026-07-28, [Security Best Practices][mcp-security] (local-server controls, state-handle hijacking, forbidden token passthrough).
- MCP specification 2026-07-28, [Caching][mcp-caching] (`ttlMs` and `cacheScope` required on complete discovery results).
- MCP specification 2025-11-25, [Transports][mcp-legacy] - **legacy** handshake/session shape, for deployments not yet on 2026-07-28.
- [Azure App Service authentication][azure-auth] (protected resource metadata preview, `WEBSITE_AUTH_PRM_DEFAULT_WITH_SCOPES`, comma-separated scopes).
- [nginx rewrite module][nginx-rewrite] (`if` and `return` directives).
- [nginx core module][nginx-core] (embedded `$http_name` variables).
- [nginx map module][nginx-map] (Origin allowlist map).
- [MCP specification versioning][mcp-versioning] (2026-07-28 is the Current revision; 2025-11-25 and earlier are Final, handshake-based revisions).
- [curl manual][curl-manual] (leading `-q`, header files, `exitcode` and `errormsg`, both added in curl 7.75.0).

[mcp-http]: https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http
[mcp-base]: https://modelcontextprotocol.io/specification/2026-07-28/basic/index
[mcp-discover]: https://modelcontextprotocol.io/specification/2026-07-28/server/discover
...
[mcp-auth-discovery]: https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization/authorization-server-discovery
...
[azure-auth]: https://learn.microsoft.com/en-us/azure/app-service/overview-authentication-authorization
