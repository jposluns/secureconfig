...
On an authorized Hub configured with the section 11 settings, exercise the signup form at `/hub/signup` and the login form (these Hub paths sit under `JupyterHub.base_url`, so a section 8 prefix makes them `/jupyter/hub/...`), varying one control at a time. Test the length floor and the common-password check separately, because under the section 11 settings NativeAuthenticator accepts a signup password only when it clears both (its `is_password_strong` requires `len(password) >= minimum_password_length` and, when `check_common_password` is on, that the password is not in the list): a password that meets the length floor but appears in the common-password list is rejected only when `check_common_password` is on and accepted when it is off, which isolates that control (lower `minimum_password_length` for this case if no listed password is long enough), while a password below the floor is rejected on length alone; with the shipped defaults (`minimum_password_length` effectively `0` since its `.tag(default=1)` annotation is metadata rather than the trait default, and `check_common_password` `False`) even an empty password is accepted. Confirm each rejection is the server's signup response, not a client-side hint. For lockout, verify the failure threshold and the cooldown length separately, because a zero cooldown is not the same as a disabled lockout: with `allowed_failed_logins = 0` the account is never throttled, while with a positive threshold the account is blocked after that many consecutive failures. An immediate retry, within the same second as the last failure, is refused at both `0` and `1200`, so it does not distinguish them; instead submit the correct password about two seconds after the last failure, when it is accepted at the effective-`0` default but still refused at `seconds_before_next_try = 1200`. The lockout also does not stop password guessing against a not-yet-approved account: while such an account is blocked, the login handler still returns a distinct authorization-pending message for a correct password and the generic error for a wrong one, a password-confirmation oracle the lockout leaves open. To exercise the approval hold in isolation from Hub admission, set `allow_all = True` with an empty `allowed_users` and sign up a fresh non-admin username: with `open_signup` off it is created unauthorized and cannot log in until an administrator approves it on `/hub/authorize`, whereas `open_signup = True` admits it at signup. Then confirm the reverse exposure by adding a not-yet-registered username to `allowed_users` and observing that its first signup is authorized immediately without approval. With `enable_signup = False`, a GET or an otherwise valid POST to the signup route is refused (HTTP 404) and creates no account, while with signup enabled the same form renders and accepts a valid registration. Read the stored credential directly (the `password` column of NativeAuthenticator's `users_info` table in the Hub database) to confirm it is a bcrypt hash, not reversible text. A wrong username, an unmigrated database, or a stale browser session is inconclusive. See the [NativeAuthenticator configuration](https://raw.githubusercontent.com/jupyterhub/nativeauthenticator/1.3.0/nativeauthenticator/nativeauthenticator.py) and the [signup and authorization handlers](https://raw.githubusercontent.com/jupyterhub/nativeauthenticator/1.3.0/nativeauthenticator/handlers.py).
...
Section 5 configures `PAMAuthenticator`, which delegates credentials to the host's user accounts and so inherits the operating system's password and lockout policy. A deployment that instead manages its own login names and passwords in the Hub, rather than authenticating against host accounts, commonly reaches for the separate [NativeAuthenticator](https://native-authenticator.readthedocs.io/en/1.3.0/quickstart.html) package (`c.JupyterHub.authenticator_class = 'native'`). It stores each password as a bcrypt hash in the Hub database and adds its own signup form, so the host no longer supplies the login password policy and NativeAuthenticator's own settings become the only floor. NativeAuthenticator replaces the host only as the credential store for login; a spawner such as section 5's `LocalProcessSpawner` may still require a matching Unix account to run each user's server. Those defaults are permissive, and two of them are weaker than the defaults their own source declarations annotate, so set them explicitly. This section applies only when NativeAuthenticator is the authenticator; under `PAMAuthenticator` the operating system owns these controls.
...
Set each of these explicitly; for two of them the shipped runtime defaults are weaker than the `.tag(default=...)` annotations in their source declarations suggest. `minimum_password_length` and `seconds_before_next_try` are declared as `Integer(...).tag(default=1)` and `.tag(default=600)`, but that tag is traitlets metadata, not the trait's default value: an `Integer` with no explicit default is `0`, so both effectively default to `0`. For `seconds_before_next_try` the package's own help text and documentation also state the 600-second default that the code does not implement; for `minimum_password_length` the documentation is accurate that there is no strength check by default. A `minimum_password_length` of `0` accepts even an empty password, since the check applied when a password is set is `len(password) >= minimum_password_length`; and a `seconds_before_next_try` of `0` leaves essentially no cooldown after a block. `check_common_password` and `allowed_failed_logins` instead default to the permissive values their declarations annotate (`False` and `0`): with common-password checking off any listed password is allowed, and with `allowed_failed_logins` at `0` lockout is disabled and the login endpoint can be brute-forced until it is set. Enabling `check_common_password` rejects a new password found in the package's bundled common-password list (its 10,000-entry `common-credentials.txt`), a floor against the most obvious choices, not a breach-corpus check. Pair `allowed_failed_logins` with `seconds_before_next_try` to throttle repeated login attempts after that many consecutive failures for one account. Some limits are worth knowing: the failure counters are held in memory, so restarting the Hub clears them; the wait compares whole elapsed seconds with `timedelta.seconds`, which ignores elapsed days, so an uncleared counter can re-block a returning user for the wait period at each 24-hour multiple until a successful login or a restart clears it; and the lockout throttles the login itself, not password guessing against a not-yet-approved account, because the login handler still returns a distinct authorization-pending message, rather than the generic error, when a pending account's password is correct, so rate-limit the login endpoint at the proxy or ingress if that matters, matching its actual path: `/hub/login`, or the prefixed form such as `/jupyter/hub/login` when `JupyterHub.base_url` sets a prefix as in section 8. These password rules are applied whenever a password is set, at signup or through a password change, so an account keeps a previously accepted password until it is changed and existing passwords are not re-checked against a newly raised policy. See the [NativeAuthenticator configuration](https://raw.githubusercontent.com/jupyterhub/nativeauthenticator/1.3.0/nativeauthenticator/nativeauthenticator.py).
...
Keep `open_signup` at its default `False`: with it off, a pending account waits for an administrator to approve it on the Hub's authorization panel (`/hub/authorize`; these `/hub/...` paths sit under `JupyterHub.base_url`, so a section 8 prefix makes them `/jupyter/hub/...`). Never set it to `True` on an internet-reachable Hub, where it would pre-authorize every self-registration, so anyone who reaches the signup form obtains a pre-authorized account (its login is still subject to the Hub's `check_allowed`). One case authorizes an account at signup even with `open_signup` off: NativeAuthenticator pre-authorizes any username already present in the section 5 `allowed_users` or `admin_users` (its `create_user` sets `pre_authorized = self.open_signup or username in self.get_authed_users()`, and `get_authed_users` includes `allowed_users` and `admin_users`). JupyterHub's own admission (`check_allowed`) independently lets a user log in only when `allow_all` is set or the username is in `allowed_users`, so under the section 5 configuration in this guide (`allow_all = False` with a populated `allowed_users`) a listed username that has not yet registered can be claimed at the signup form, with a password of the claimant's choosing, by anyone who can reach it, arriving immediately authorized without the approval step ever running. Approval still gates an account that registered before its name was listed, since adding a name to `allowed_users` does not retroactively authorize an existing pending record, and an administrator can revoke an account's authorization. Treat the signup form as reachable only by trusted users, register the `allowed_users` accounts deliberately, and if self-service registration is not wanted at all set `enable_signup = False`, which makes the signup route reject every request. Passwords are stored as bcrypt hashes in the Hub database, so protect that database and its backups as a credential store.
...
- NativeAuthenticator quickstart and authenticator_class: https://native-authenticator.readthedocs.io/en/1.3.0/quickstart.html
- NativeAuthenticator request handlers (signup 404 when signup is disabled, and the authorization panel): https://raw.githubusercontent.com/jupyterhub/nativeauthenticator/1.3.0/nativeauthenticator/handlers.py
