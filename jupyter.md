...
On an authorized Hub configured with the section 11 settings, exercise the signup form at `/hub/signup` and the login form, holding the section 5 admission settings constant and varying one control at a time. With the hardened configuration, a signup password shorter than `minimum_password_length` or one present in the common-password list is rejected at the form, while with the defaults (`minimum_password_length` `1`, `check_common_password` `False`) the same passwords are accepted; confirm the rejection is the server's signup response, not a client-side hint. After `allowed_failed_logins` consecutive failed logins for one account, the next attempt within `seconds_before_next_try` is refused even with the correct password, whereas with the default `0` the attempts are never throttled. A freshly self-registered account cannot log in until an administrator authorizes it in the Hub admin view, whereas turning on `open_signup` admits it immediately; verify the pre-authorization state denies access rather than inferring it. Inspect the stored record to confirm the password is held as a bcrypt hash, not reversible text. A wrong username, an unmigrated database, or a stale browser session is inconclusive. See the [NativeAuthenticator configuration](https://raw.githubusercontent.com/jupyterhub/nativeauthenticator/1.2.0/nativeauthenticator/nativeauthenticator.py).
...
`minimum_password_length` defaults to `1`, so a single-character password is accepted until this is raised. `check_common_password` defaults to `False`; enabling it rejects a new password found in the package's bundled common-password list, which is a floor against the most obvious choices, not a breach-corpus check. `allowed_failed_logins` defaults to `0`, which disables lockout entirely, so leaving it unset lets the login endpoint be brute-forced; set it together with `seconds_before_next_try` (default `600` seconds) to bound repeated guesses after that many consecutive failures for one account. These password rules are applied when an account is created at signup, so accounts that already exist keep whatever password they were given and are not re-checked against a newly raised policy. See the [NativeAuthenticator configuration](https://raw.githubusercontent.com/jupyterhub/nativeauthenticator/1.2.0/nativeauthenticator/nativeauthenticator.py).

Control who may register. NativeAuthenticator's signup form is enabled by default (`enable_signup` is `True`), and a self-registered account stays unauthorized until an administrator approves it in the Hub, unless `open_signup` is turned on:

```python
c.NativeAuthenticator.open_signup = False
```

`open_signup` defaults to `False`, which is the value to keep: every self-registered account then waits for an administrator's approval before it can log in. Never set it to `True` on an internet-reachable Hub, where it would let anyone who can reach the signup form obtain an active, logged-in account without approval. If self-service registration is not wanted at all, set `enable_signup = False` and provision accounts administratively. Because NativeAuthenticator authenticates against the Hub's own store rather than host accounts, review how its approval workflow composes with the section 5 admission settings (`allow_all`, `allowed_users`) for your Hub version rather than assuming either one alone gates access. Passwords are stored as bcrypt hashes in the Hub database, so protect that database and its backups as a credential store.
...
- NativeAuthenticator configuration traits and password, lockout, and signup enforcement: https://raw.githubusercontent.com/jupyterhub/nativeauthenticator/1.2.0/nativeauthenticator/nativeauthenticator.py
