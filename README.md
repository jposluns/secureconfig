...
3. Apply every guide that fits the deployment, not just one. Select the service or framework, its host or platform, containers where used, plus identity, secrets, and the operational controls below. For a service with no guide, run the alignment assessment in [controls-reference.md](controls-reference.md), which distils the control patterns common to every guide, and verify the service's native controls against the vendor documentation; then request a guide by adding a file to [requests/](requests/README.md) so the service is covered next time.
