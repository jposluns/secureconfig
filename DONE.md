...
| 3.9 | `VERSION` names the most recently merged pull request, so a bookkeeping pull request must carry its own number, and out-of-order merges make the value go backwards | Done, #55. `CONTRIBUTING.md` gained a "Shipping a change" section: open the pull request, write its number into `VERSION`, push, then merge; merge in numeric order and where that is not possible the later pull request sets the value; record the change in the same pull request. No monotonicity gate, per the maintainer's ruling |
