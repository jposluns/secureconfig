...
- **mTLS on the servers.** The authentication docs describe `http_tls_config` and `grpc_tls_config`
  ...
  header. Plain HTTP got `400`. With exactly this shape (HTTP mTLS; gRPC, `instance_addr` and an
  `inmemory` ring on 127.0.0.1), a push with a client certificate returned `204` and a query returned
  the pushed line with `200`, and Loki held no listener but its HTTP and gRPC ports. `instance_addr`
  matters: the rings advertise it and internal traffic dials it, so if it resolves to a routable address
  while gRPC listens on loopback, ingestion breaks (inferred from the source, not run here). Keep gRPC
  off the network: left at its default it listens on every interface in plaintext. With `memberlist` as
  the ring store, the same run opened a gossip listener (TCP only) on `memberlist.bind_addr`; bind that
  privately, or set `memberlist.tls-enabled`, if you need it. Do not add `grpc_tls_config` to a single
  ...
  because its own components call that gRPC server in plaintext. A deployment whose components talk gRPC
  across hosts needs TLS on every component's gRPC client as well; that was not demonstrated here, so
  take it from the Loki configuration reference for your version. mTLS does not set `X-Scope-OrgID`, so
  your agent or proxy still does.
