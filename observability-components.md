...
- **mTLS on the servers.** The authentication docs describe `http_tls_config` and `grpc_tls_config`
  ...
  because Loki copies it into its ring and component addresses (ingester, distributor, query frontend
  and others; `config_wrapper.go`); if it resolves to a routable address while gRPC listens on loopback,
  ingestion breaks (inferred from that source, not run here). `replication_factor` defaults to 3
  (`common.go`), more ingesters than one instance has. Keep gRPC off the network: left at its default it
  ...
  return `500` and queries hang, and Loki logged `error reading server preface`, which indicates that
  its own components call that gRPC server in plaintext (inferred from the logged error, so the warning
  extends to other `client_auth_type` values). A deployment whose components talk gRPC across hosts
  needs TLS on every component's gRPC client as well; that was not demonstrated here, so take it from
  the Loki configuration reference for your version. mTLS does not set `X-Scope-OrgID`, so your agent or
  proxy still does.
