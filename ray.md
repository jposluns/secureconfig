...
ss -tlnp   # read every listener; 10001 should show the node's private address (plus a loopback listener, per the source), but 6379 can show every
...
- Ray Client server listeners, node address plus loopback (pinned tag): https://github.com/ray-project/ray/blob/ray-2.58.0/python/ray/util/client/server/server.py#L799-L801
- Ray Client proxier listeners, node address plus loopback (pinned tag): https://github.com/ray-project/ray/blob/ray-2.58.0/python/ray/util/client/server/proxier.py#L929-L931
