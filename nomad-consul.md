...
registration; the agent's debug endpoints still need `enable_debug`.
...
returns `200`. Each agent applies its own `acl` block, so any agent that answers `/v1/agent/self`
...
the anonymous token, and the probe then prints a result this guide does not list. Set
