...
`/v1/jobs` returns `200`. With ACLs on, `/v1/acl/token/self` returns `200` with
`"AccessorID":"anonymous"` and `/v1/jobs` returns `403` `Permission denied`, but that `403` covers
listing jobs in the default namespace only, and it looks the same before bootstrap, so fixed needs
two more checks on a server. First, `nomad acl bootstrap` must fail with an error that includes "ACL
...
its rights from the policy named `anonymous`. A connection failure prints curl's error and exit code, and says nothing about ACLs. Any other
...
ports. Before bootstrap, the probe already printed the ACLs-on results; the first `nomad acl bootstrap`
...
roles, or service, node or templated identities. A connection failure prints curl's error and exit code, and says nothing about ACLs. Any other
status or message is inconclusive: a redirect (for example from a trailing slash, or from HTTP to
HTTPS), a proxy's own response, or a certificate error once TLS is on says nothing about the
server's ACLs; run the block from a host that trusts the cluster's CA.
