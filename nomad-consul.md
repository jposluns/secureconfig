...
2.0.7 and Consul 2.0.4, Community Edition; Enterprise namespaces and partitions are not covered.
...
The first two paths are answered by the servers, so only `/v1/agent/self` shows an agent whose own
...
its rights from the policy named `anonymous`. Set the token without leaving it in shell history, for
example with `read -rs NOMAD_TOKEN && export NOMAD_TOKEN`, and `unset NOMAD_TOKEN` afterwards; an
exported variable can be read from `/proc` by the same account and by root. A connection failure prints curl's error and exit code, and says nothing about ACLs. Any other
...
returns `200`. With ACLs on and `default_policy = "deny"`, `/v1/agent/self` returns `403`
...
roles, or service, node or templated identities. Set and unset `CONSUL_HTTP_TOKEN` the same way. A connection failure prints curl's error and exit code, and says nothing about ACLs. Any other
status or message is inconclusive: a redirect (for example from a trailing slash, or from HTTP to
HTTPS), a proxy's own response, or a certificate error once TLS is on says nothing about the
server's ACLs; run the block from a host that trusts the cluster's CA.

All of these outcomes were observed with this block and these commands, on agents listening on test
ports. Before bootstrap, the probe already printed the deny-policy results; the first `consul acl
