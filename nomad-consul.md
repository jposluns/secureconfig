...
Put the same block on every agent, servers and clients: each agent applies its own ACL settings.
...
these commands, by running them in a subshell, for example
...
For Consul, send two requests without a token, to every agent's HTTP API, servers and clients. Substitute the HTTP API URL (for example
...
returns `200`. Each agent applies its own ACL settings, so any agent that answers `/v1/agent/self`
...
roles, or service, node or templated identities. Set `CONSUL_HTTP_TOKEN` the same way, in a subshell around the command. A connection failure prints curl's error and exit code, and says nothing about ACLs. Any other
status or message is inconclusive: a redirect (for example from a trailing slash, or from HTTP to
HTTPS), a proxy's own response, or a certificate error once TLS is on says nothing about the
server's ACLs; run the block from a host that trusts the cluster's CA.
