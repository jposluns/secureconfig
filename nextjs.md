...
On Vercel and similar platforms the platform terminates TLS; nothing to configure ([paas.md](paas.md)). Self-hosted, `next start` listens on every interface on port 3000 by default (it passes no hostname to Node's `listen`, which binds `::` where IPv6 is available and `0.0.0.0` otherwise; `next start --help` still describes the default as `0.0.0.0`); bind to loopback and put a reverse proxy in front, which the Next.js self-hosting guide itself recommends, with TLS and headers per [nodejs.md](nodejs.md), [nginx.md](nginx.md), or [caddy.md](caddy.md).
...
# POSITIVE controls, so a denial above is the auth check firing and not a route broken for everyone. Set
# VALID_COOKIE to a real, freshly issued cookie VALUE and COOKIE_NAME to its name - this guide's example uses
# `session`, Better Auth uses `better-auth.session_token` (or `__Secure-better-auth.session_token` with
# secure cookies). Test the admin route with an admin AND a non-admin cookie (authorization, not only
# authentication), and confirm each success body is the real protected content, not a login page:
if [ -z "${VALID_COOKIE:-}" ] || [ -z "${COOKIE_NAME:-}" ]; then
  echo "inconclusive: set COOKIE_NAME and VALID_COOKIE first"
else
  curl -q -sS --noproxy '*' -o dash-auth.txt  -w 'http=%{http_code}\n' -b "$COOKIE_NAME=$VALID_COOKIE" https://app.example.com/dashboard   # expect 200; dash-auth.txt is the dashboard, not /login
  curl -q -sS --noproxy '*' -o admin-auth.txt -w 'http=%{http_code}\n' -b "$COOKIE_NAME=$VALID_COOKIE" https://app.example.com/api/admin    # admin cookie: 200; a non-admin cookie here must be 403 or a /login redirect
fi
...
if [ -z "${SESSION_SECRET:-}" ] || [ ! -d .next/static ]; then
  echo "inconclusive: set SESSION_SECRET and run next build first"
else
  if grep -rIF -- "$SESSION_SECRET" .next/static; then rc=0; else rc=$?; fi
  case "$rc" in
...
- Next.js `next start` declares `-H, --hostname` with help text only and no default value, unlike `--port`'s `.default(3000)` just above it (pinned tag v16.3.6): https://github.com/vercel/next.js/blob/v16.3.6/packages/next/src/bin/next.ts#L447-L458
