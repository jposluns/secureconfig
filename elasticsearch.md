...
- The official Docker images (8.19.22, 9.4.6 and 9.5.3 at the pinned library commit) ship an `elasticsearch.yml` that sets `network.host: 0.0.0.0`, which Elastic documents as "the addresses of all available network interfaces". So unless an environment variable, a `-E` setting passed as an argument, or a bind-mounted configuration file overrides it (for example with `transport.host`), transport (9300) listens on them as well as HTTP (9200). Keep security auto-configuration, which sets up transport TLS, and never publish 9300.
