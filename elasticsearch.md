...
- The official Docker images (8.19.22, 9.4.6 and 9.5.3 at the pinned library commit) ship an `elasticsearch.yml` that sets `network.host: 0.0.0.0`, which Elastic documents as "the addresses of all available network interfaces". So unless an environment variable, a `-E` setting given after `elasticsearch` in the container command (the entrypoint passes those through; any other first argument runs as its own command), or a bind-mounted configuration file overrides it (for example with `transport.host`), transport (9300) listens on them as well as HTTP (9200). Keep security auto-configuration, which sets up transport TLS, and keep 9300 unreachable: do not publish it (`-P` would, because the image EXPOSEs it), do not run the container with host networking, and do not share its network with untrusted containers.
