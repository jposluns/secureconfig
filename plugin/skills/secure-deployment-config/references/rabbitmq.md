...
Do not loosen the guest account's localhost restriction; in a container from the official image as shipped, where `10-defaults.conf` loosens it, delete `guest`, wherever it exists, as above. Exported definitions contain password hashes and hashing metadata; keep exports access-restricted and out of images and version control ([secrets.md](secrets.md)). Definitions imports are privileged provisioning input: a stale or untrusted file can introduce accounts, grants, policies, or plugin parameters.
