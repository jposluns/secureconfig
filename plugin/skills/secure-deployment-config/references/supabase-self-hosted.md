...
key is the opposite: it is meant to be public and is constrained by row-level security. That protection depends on both SQL grants and RLS: for an ordinary API role, access needs an applicable
schema/table grant AND a matching RLS policy. With RLS enabled and no applicable policy an ordinary role
reads no rows (a successful, empty `SELECT`, not an authentication error); with RLS never enabled, the
...
(                              # a subshell, so your own script arguments are untouched
  set -- PASTE_WHOLE_BLOCK 'REPLACE_WITH_BASE_URL' 'REPLACE_WITH_A_FUNCTION_NAME'
  ...
  base=${1%/}
  # Unauthenticated: with FUNCTIONS_VERIFY_JWT=false the main worker runs the function and answers (http=200 or
  # a function error, not 401). A lone 401 could be the function's OWN check, so pair it with an authorized call
  # (a valid CURRENT anon key, which is public by design) that must return the function's known response.
  curl -q -g -sS --noproxy '*' --connect-timeout 5 --max-time 20 \
    -w '\nno-key http=%{http_code} exit=%{exitcode} err=%{errormsg}\n' "$base/functions/v1/$2"
  printf 'apikey: %s\n' 'REPLACE_WITH_CURRENT_ANON_KEY' | curl -q -g -sS --noproxy '*' --connect-timeout 5 --max-time 20 \
...
disable shell tracing). The shipped API keys still work if a request to `$base/auth/v1/settings` carrying the
demo `ANON_KEY` (or `SERVICE_ROLE_KEY`) in `apikey` returns settings JSON rather than a rejection; pair each with
a current key that must be accepted, so a gateway rejection cannot mask backend acceptance. The signing SECRET
itself is only proven rotated by a forged token (reasoned, backlog row 2.27): mint an unexpired HS256
`service_role` JWT signed with the public demo `JWT_SECRET`, send it as `Authorization: Bearer` alongside a
valid current `apikey` against a harmless REST canary, and require the backend to REJECT it (an accepted read
means the demo secret is still trusted). The Studio dashboard still accepts the default Basic auth if the
