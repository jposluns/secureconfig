...
than just the shipped keys swapped. There are two layers to check. First, from an external host none of these ports should answer at all: a
connection refused or a timeout during the connect is the hardened state, and any HTTP or database
response means the port is published and is itself the finding, whatever the credentials do next. The
...
For a port you can still reach, two more checks show whether the shipped credentials were replaced. The
Studio dashboard still accepts the default basic-auth if the gateway answers a request carrying
`supabase:this_password_is_insecure_and_should_be_updated` (a `-u` argument against the gateway on
`8000`, or your proxy) with `http=200` rather than `401`. The database still accepts the demo password
if `psql` connects to the Supavisor pooler on `5432` or `6543` with the demo `POSTGRES_PASSWORD`, as
user `postgres.your-tenant-id` (the demo `POOLER_TENANT_ID`) and database `postgres`; once the password
is regenerated that connection fails authentication.
