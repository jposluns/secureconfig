...
- First run auto-generates the root password into `/etc/gitlab/initial_root_password` (read it with `docker exec -it gitlab grep 'Password:' /etc/gitlab/initial_root_password`); GitLab deletes the file in the first container restart after 24 hours. Sign in and change it before anyone else can reach the instance, or preset it with `gitlab_rails['initial_root_password']` in `gitlab.rb` or `GITLAB_OMNIBUS_CONFIG` (there is no `GITLAB_ROOT_PASSWORD` variable in standard deployments).
- `signup_enabled` defaults to `true`, so any visitor can register. New instances hold registrants pending approval (`require_admin_approval_after_user_signup` is on by default), but for a private forge clear "Allow new user accounts" under Admin > Settings > General (`signup_enabled = false`) rather than relying on approval discipline.
...
- GitLab can enforce two-factor authentication for every user from Admin > Settings; turn it on so local accounts are not password-only.
...
if docker exec gitlab test -f /etc/gitlab/initial_root_password; then
  echo 'initial_root_password present: rotate root and remove it'
else
  echo 'initial_root_password absent'
fi
```
