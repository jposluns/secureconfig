...
The Keycloak realm had three users, each with an `hd` claim from a user-attribute mapper standing in for Google's:
...
- **oauth2-proxy with Keycloak, restricted to `example.com` (fixed):**
  ...
  - The unverified user got an error page, with oauth2-proxy logging "email in id_token … isn't verified".
- **Native `st.login` with section 2's gate** (the guide's code, with only the final `Hello` line replaced by the canary):
  ...
  - The user in another domain saw "This account is not authorized for this app."
  - The unverified user saw "This account's email is not verified."
  - Neither of those two saw the canary.
- **Native `st.login` exposed states:**
  - With only the `st.login()` check and no `hd` or email check, the user in another domain rendered the canary.
