...
| 3306 | MySQL and MariaDB (the official images, MySQL 8.4, 9.7 and innovation and MariaDB 10.6 through 13.1 checked, listen on every address unless an argument or option file sets a bind address or `skip_networking`) | Varies by install method; MariaDB 10.4+ local root via Unix socket; on a new data directory the official images create a root account for the root host (`root@'%'` when it is unset or empty; none beyond local root when it is `localhost`), with an empty password when the empty-password variable is set and no root password, hash or random password is requested | [mysql.md](mysql.md), [cloud-firewalls.md](cloud-firewalls.md) |
