...
| 7947 | Weaviate's internal cluster API, on every local address | None unless `CLUSTER_BASIC_AUTH_USERNAME` or `CLUSTER_BASIC_AUTH_PASSWORD` is non-empty, then HTTP basic authentication on its data and management handlers (a descriptive `/` route stays open) | [vector-databases.md](vector-databases.md) |
...
| 9001 | MinIO's web console (the `--console-address` / `MINIO_CONSOLE_ADDRESS` port, conventionally 9001); also Mosquitto WebSockets, and a Helicone Compose backing-service host publication | `minioadmin`/`minioadmin` when root variables unset | [minio.md](minio.md), [mosquitto.md](mosquitto.md), [llm-observability.md](llm-observability.md) |
