...
| 8123 | ClickHouse HTTP, plaintext; also the Helicone Compose ClickHouse's container port (published on host port 18123) | Base config: `default` user with an empty password, allowed to connect from any address; packaging can change it | [clickhouse.md](clickhouse.md), [llm-observability.md](llm-observability.md) |
