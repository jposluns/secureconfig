...
| 7946 | Grafana Loki memberlist gossip when memberlist is the ring store, on 0.0.0.0 by default, over TCP; also Weaviate's memberlist gossip, on 0.0.0.0 unless `CLUSTER_BIND_ADDR` is set | not stated | [observability-components.md](observability-components.md), [vector-databases.md](vector-databases.md) |
| 7946/UDP | Weaviate's memberlist gossip, on 0.0.0.0 unless `CLUSTER_BIND_ADDR` is set | not stated | [vector-databases.md](vector-databases.md) |
...
| 8300 | Consul server RPC, on `bind_addr` (every address by default); also Weaviate's Raft, on `CLUSTER_BIND_ADDR`, else its advertised address | not stated | [nomad-consul.md](nomad-consul.md), [vector-databases.md](vector-databases.md) |
