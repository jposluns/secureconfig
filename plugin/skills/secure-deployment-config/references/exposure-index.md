...
| 1025, 8025 | Helicone Compose backing-service host publications; the guide names MailHog among the backing services but does not assign a protocol to each of these ports | [llm-observability.md](llm-observability.md) |
...
| 1777 | OpenTelemetry Collector pprof diagnostic extension, when enabled | [llm-observability.md](llm-observability.md) |
...
| 2019 | Caddy admin API, which requires no credentials and defaults to localhost | [caddy.md](caddy.md) |
...
| 2746 | Argo Server (Argo Workflows API and UI) | [workflow-orchestrators.md](workflow-orchestrators.md) |
...
| 3389 | RDP, mentioned only as an omitted inventory check in the cloud-firewall guide | [cloud-firewalls.md](cloud-firewalls.md) |
| 3478 | coturn STUN/TURN over UDP and TCP | [realtime-voice-infra.md](realtime-voice-infra.md) |
...
| 5001 | Dify backend API, in the guide's in-container request example | [agent-builders.md](agent-builders.md) |
...
| 5349 | coturn TLS listener; DTLS is opt-in since 4.17.0, and this port number does not prove encryption is mandatory | [realtime-voice-infra.md](realtime-voice-infra.md) |
...
| 5901 | VNC in a GPU desktop template example; an example port, not a platform-wide default | [gpu-clouds.md](gpu-clouds.md) |
...
| 6060 | A Go service's pprof and expvar diagnostics listener, bound to loopback in the guide's example | [go.md](go.md) |
| 6080 | noVNC web interface in a GPU desktop template example; an example port, not a platform-wide default | [gpu-clouds.md](gpu-clouds.md) |
| 6222 | NATS cluster routes, a separate listener with its own authentication and TLS | [nats.md](nats.md) |
...
| 6789 | Metrics port mentioned only in the realtime voice guide's Verify inventory; service attribution and default status are not established there | [realtime-voice-infra.md](realtime-voice-infra.md) |
| 7000 | frp server | [tunnels.md](tunnels.md) |
| 7222 | NATS gateways, a separate listener with its own authentication and TLS | [nats.md](nats.md) |
| 7233 | Temporal frontend gRPC | [workflow-orchestrators.md](workflow-orchestrators.md) |
| 7422 | NATS accepted leafnode connections, a separate listener with its own authentication and TLS | [nats.md](nats.md) |
| 7473, 7474, 7687 | Neo4j HTTPS, HTTP, and Bolt | [neo4j.md](neo4j.md) |
| 7700 | Meilisearch HTTP API; the binary defaults to localhost, while the official Docker image binds all interfaces | [search-engines.md](search-engines.md) |
| 7800 | Keycloak clustered cache transport, carrying sessions and tokens; TLS is on by default for TCP stacks | [self-hosted-idp.md](self-hosted-idp.md) |
...
| 7880 | LiveKit HTTP/WebSocket signaling and API, which need a TLS proxy | [realtime-voice-infra.md](realtime-voice-infra.md) |
| 7881 | LiveKit ICE media TCP mux, separate from signaling | [realtime-voice-infra.md](realtime-voice-infra.md) |
| 7882/UDP | LiveKit development-mode media mux; outside development mode the documented default is the 50000 to 60000 media range | [realtime-voice-infra.md](realtime-voice-infra.md) |
| 8000 | SurrealDB, Chroma, Triton HTTP, Coolify, Vaultwarden outside Docker, Portainer's Edge agent tunnel, the Supabase self-hosted API gateway (Kong) which fronts the whole stack, or GlitchTip's web, API, and open self-signup | [surrealdb.md](surrealdb.md), [vector-databases.md](vector-databases.md), [model-servers.md](model-servers.md), [devops-uis.md](devops-uis.md), [supabase-self-hosted.md](supabase-self-hosted.md), [self-hosted-error-trackers.md](self-hosted-error-trackers.md) |
| 8001, 8002 | Triton gRPC and Triton Prometheus metrics; and Datasette's CLI default HTTP listener on 8001, loopback by default (an overridden bind or a published container mapping is the exposure) | [model-servers.md](model-servers.md), [sqlite-http-frontends.md](sqlite-http-frontends.md) |
| 8055 | Directus HTTP API and admin app | [headless-cms-instant-api.md](headless-cms-instant-api.md) |
| 8080 | llama.cpp, Weaviate HTTP, Airflow, code-server, Open WebUI's container port, Dify's nginx when mapped to `127.0.0.1:8080`, Spring Boot, Go, and the Vast.ai Jupyter deployment; Keycloak's HTTP port, which exists only when `--http-enabled=true`; and Hasura GraphQL Engine v2, which binds every interface by default; and Argo CD's argocd-server pod listener, fronted by its Service on 80 and 443; and sqlite-web's CLI default HTTP listener, loopback by default (an overridden bind or a published container mapping is the exposure) | [model-servers.md](model-servers.md), [vector-databases.md](vector-databases.md), [workflow-orchestrators.md](workflow-orchestrators.md), [code-server.md](code-server.md), [open-webui.md](open-webui.md), [agent-builders.md](agent-builders.md), [java.md](java.md), [go.md](go.md), [gpu-clouds.md](gpu-clouds.md), [self-hosted-idp.md](self-hosted-idp.md), [headless-cms-instant-api.md](headless-cms-instant-api.md), [gitops-controllers.md](gitops-controllers.md), [sqlite-http-frontends.md](sqlite-http-frontends.md) |
...
| 8083 | Argo CD argocd-server metrics, separate from its API and UI listener | [gitops-controllers.md](gitops-controllers.md) |
...
| 8090 | PocketBase `serve`, which binds `127.0.0.1:8090` when no domain is given | [pocketbase.md](pocketbase.md) |
...
| 8107 | Typesense peering service, which should be restricted to cluster members | [search-engines.md](search-engines.md) |
| 8108 | Typesense API, which binds all interfaces by default | [search-engines.md](search-engines.md) |
...
| 8400, 8401 | VictoriaMetrics cluster native RPC channels, separate from vmstorage's HTTP maintenance and metrics listener | [time-series-metrics-stores.md](time-series-metrics-stores.md) |
| 8404 | HAProxy HTTPS stats listener in the configured example; a chosen port, and stats are disabled unless configured | [haproxy.md](haproxy.md) |
...
| 8432 | Mem0 Compose PostgreSQL publication: host port 8432 maps to container port 5432 | [ai-infra-services.md](ai-infra-services.md) |
...
| 8812 | QuestDB PostgreSQL wire protocol, shipping the default credential `admin` / `quest` | [time-series-metrics-stores.md](time-series-metrics-stores.md) |
...
| 9004, 9005, 9010 | ClickHouse MySQL compatibility, PostgreSQL compatibility, and interserver replica traffic over HTTPS (the HTTP interserver port 9009 is listed separately) | [clickhouse.md](clickhouse.md) |
...
| 9090 | InvokeAI, and the Prometheus server, which listens on `0.0.0.0:9090` by default with no authentication | [image-gen-uis.md](image-gen-uis.md), [admin-uis.md](admin-uis.md) |
...
| 9094 | Kafka KRaft controller listener in the guide's configured SASL_SSL example; the port is chosen, not fixed | [kafka.md](kafka.md) |
| 9095 | Kafka's `INTERNAL` listener in the guide's example configuration, on a private address | [kafka.md](kafka.md) |
| 9200 to 9300 | Elasticsearch HTTP default range, binding the first free port; OpenSearch HTTP on 9200 | [elasticsearch.md](elasticsearch.md) |
...
| 9292 | Puma standalone, whose default bind is all interfaces; and the Flux notification-controller webhook receiver, in-cluster behind the `webhook-receiver` Service | [ruby.md](ruby.md), [gitops-controllers.md](gitops-controllers.md) |
| 9300 to 9400 | Elasticsearch transport default range; authentik's unauthenticated Prometheus metrics on 9300 | [elasticsearch.md](elasticsearch.md), [self-hosted-idp.md](self-hosted-idp.md) |
...
| 9641 | coturn Prometheus metrics, disabled by default; when enabled, it binds a wildcard address and serves without authentication | [realtime-voice-infra.md](realtime-voice-infra.md) |
...
| 11435 | The Ollama guide's local policy proxy, an nginx listener on `127.0.0.1:11435` for the inference-only deployment | [ollama.md](ollama.md) |
| 13133 | OpenTelemetry Collector health-check extension, when enabled | [llm-observability.md](llm-observability.md) |
| 15671 | RabbitMQ management UI over TLS, as noted in the guide's listener inventory | [rabbitmq.md](rabbitmq.md) |
| 15672 | RabbitMQ management UI | [rabbitmq.md](rabbitmq.md) |
| 16379 | Redis Cluster's node-to-node bus (`cluster-port 16379` in the guide's example), which is opt-in and needs its own restriction | [redis.md](redis.md) |
| 18123 | Helicone Compose ClickHouse HTTP publication: host port 18123 maps to container port 8123; it also falls inside the Ray worker range | [llm-observability.md](llm-observability.md) |
| 19000 | Helicone Compose backing-service publication: host port 19000 maps to container port 9000, but the guide does not explicitly attribute this mapping to a service; it also falls inside the Ray worker range | [llm-observability.md](llm-observability.md) |
| 19530 | Milvus gRPC | [vector-databases.md](vector-databases.md) |
| 20202 | LiteFS HTTP replication API, including database export/import and administrative operations | [sqlite.md](sqlite.md) |
...
| 26379 | Redis Sentinel's separate listener; its shipped configuration disables protected mode and supplies no active authentication rule | [redis.md](redis.md) |
...
| 35672 to 35682 | RabbitMQ remote CLI tools' own Erlang distribution-port range (default); the guide says to restrict the actual range to the necessary peers | [rabbitmq.md](rabbitmq.md) |
...
| 54388 | Helicone Compose PostgreSQL publication: host port 54388 maps to container port 5432 | [llm-observability.md](llm-observability.md) |
| 55679 | OpenTelemetry Collector zPages diagnostic extension, when enabled | [llm-observability.md](llm-observability.md) |
| 57800 | Keycloak clustered cache failure detection; TLS is on by default for TCP stacks | [self-hosted-idp.md](self-hosted-idp.md) |
