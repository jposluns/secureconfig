...
| 7946 | Grafana Loki memberlist gossip when memberlist is the ring store, on 0.0.0.0 by default, over TCP; also Weaviate's memberlist gossip, on 0.0.0.0 unless a non-empty `CLUSTER_BIND_ADDR` is set | not stated | [observability-components.md](observability-components.md), [vector-databases.md](vector-databases.md) |
| 7946/UDP | Weaviate's memberlist gossip, on 0.0.0.0 unless a non-empty `CLUSTER_BIND_ADDR` is set | not stated | [vector-databases.md](vector-databases.md) |
