...
| 21123, 21124, 22125, 22222 | Milvus internal gRPC: query node, data node, mix coordinator and streaming node (v3.0.2 and v2.6.24), each on every interface (the query node, data node and streaming node fall back to a random port, still on every interface, when theirs is taken) | No credentials checked; internal TLS is one-way, off by default, and falls back to plaintext if its certificate does not load | [vector-databases.md](vector-databases.md) |
