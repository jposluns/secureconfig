...
| 9004, 9005, 9010 | ClickHouse MySQL compatibility, PostgreSQL compatibility, and interserver replica traffic over HTTPS (the HTTP interserver port 9009 is listed separately); also the Onyx development Compose's MinIO publications on 9004 and 9005 | SQL (9004, 9005): base config `default` user with an empty password, allowed to connect from any address; replication (9010): unauthenticated unless `interserver_http_credentials` is set, and independent of SQL users; packaging can change these | [clickhouse.md](clickhouse.md), [ai-infra-services.md](ai-infra-services.md) |
