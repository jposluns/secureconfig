...
The official Docker images change the protected-mode default. Their build patches Redis so `protected-mode` defaults to `no` (every image in the repository at the pinned commit, 6.2 through 7.4, Debian and Alpine alike), and their default command runs `redis-server` with no configuration file, so the compiled-in defaults apply: in the 7.4.8 build, `bind * -::*` (every IPv4 and IPv6 address) and a `default` user that is `on`, `nopass` and `+@all`. A container from those images, started without a configuration file or arguments of its own, therefore accepts unauthenticated commands with full access from any address that reaches it, including everything a bare `-p 6379:6379` publishes. Start it with a configuration file carrying the `bind` and credential settings in this guide, and publish only to host loopback or a private network.
...
- Official Redis Docker images: the build patch that sets the `protected-mode` default to 0, and `CMD ["redis-server"]`, present in every Dockerfile at the pinned commit and shown here for 7.4 Debian and Alpine, 7.2 and 6.2 (pinned commit 584feaab6cd12a77866c121beff1bb16ff239486): https://github.com/docker-library/redis/blob/584feaab6cd12a77866c121beff1bb16ff239486/7.4/debian/Dockerfile#L85-L90, https://github.com/docker-library/redis/blob/584feaab6cd12a77866c121beff1bb16ff239486/7.4/debian/Dockerfile#L146-L150, https://github.com/docker-library/redis/blob/584feaab6cd12a77866c121beff1bb16ff239486/7.4/alpine/Dockerfile#L79-L84, https://github.com/docker-library/redis/blob/584feaab6cd12a77866c121beff1bb16ff239486/7.2/debian/Dockerfile#L85-L90 and https://github.com/docker-library/redis/blob/584feaab6cd12a77866c121beff1bb16ff239486/6.2/debian/Dockerfile#L85-L90
- Redis 7.4.8 compiled defaults: `bind` `* -::*`, `protected-mode` 1, and the `default` user `on nopass +@all` (pinned tag 7.4.8): https://github.com/redis/redis/blob/7.4.8/src/server.h#L119-L120, https://github.com/redis/redis/blob/7.4.8/src/config.c#L3055 and https://github.com/redis/redis/blob/7.4.8/src/acl.c#L1407-L1415
