...
- PyTorch TCP rendezvous opens a TCPStore on all interfaces by default, even when its advertised master address is private or loopback. vLLM also has stateless-group stores that explicitly bind the supplied address; do not generalize that narrower bind to every store. Multi-node multiprocessing uses `--master-addr` and `--master-port` (default port 29501). Data-parallel group initialization can allocate additional ports, and Ray can allocate stores dynamically. Gloo/NCCL transports add their own communication sockets.
- Remote engine clients use unauthenticated ZMQ sockets: a startup handshake on `--data-parallel-address` and `--data-parallel-rpc-port` (default port 29550), plus request/result and coordinator sockets on dynamically allocated ports. The address defaults to loopback for the multiprocessing backend unless a master address is supplied; Ray uses the detected node IP. Co-located engine channels normally use IPC. Remote worker message queues also bind a detected node IP and a dynamic TCP port. `--headless` removes the HTTP frontend, not these internal channels.
- KV-transfer listeners are connector-specific and opt-in. For example, the NIXL handshake defaults to `localhost`, port 5600 plus the data-parallel index, while Mooncake's producer bootstrap server binds `0.0.0.0`, port 8998, and its worker side channel uses the detected node IP and a dynamic port. These paths add no peer authentication. Audit the selected connector and its transfer library; there is no universal KV-transfer bind switch.
- Optional `--grpc` replaces HTTP with an unauthenticated, unencrypted gRPC server on `--host`/`--port`, default `0.0.0.0:8000`. The security page's `--grpc-port` description does not match this release's CLI. Multi-port external data parallelism also adds a health supervisor without an API-key check on `--host`, default port 9256 (`--data-parallel-supervisor-port`), alongside one API port per local rank. The supervisor also accepts the frontend TLS options.

Set `VLLM_HOST_IP` to each node's own isolated-network IP; automatic detection can select a public-facing interface and falls back to `0.0.0.0`. Set `--data-parallel-address` to the head node's isolated-network IP and use the same `--data-parallel-rpc-port` on participating nodes. For multi-node multiprocessing, set `--master-addr` to the intended master IP too. `VLLM_PORT` is a starting point for the open-port allocator, not a firewall port range: sockets that bind port zero still receive dynamic ports. `VLLM_DP_MASTER_IP` and `VLLM_DP_MASTER_PORT` serve the environment-driven/offline DP path; they are not substitutes for the serving flags. NIXL has separate `VLLM_NIXL_SIDE_CHANNEL_HOST` and `VLLM_NIXL_SIDE_CHANNEL_PORT` settings; `VLLM_MOONCAKE_BOOTSTRAP_PORT` changes Mooncake's bootstrap port, not its wildcard bind.
...
From a trusted cluster peer and then from a host outside the allowed cluster network, run this block for each recorded internal TCP endpoint. Substitute the actual numeric node address reachable from that vantage and the observed port, not the API proxy address. It only attempts a TCP connection and sends no application payload:

```bash
(
  set -- PASTE_WHOLE_BLOCK 'REPLACE_WITH_NODE_IP' 'REPLACE_WITH_INTERNAL_PORT'
  ...
  case "$1:$2" in
    *REPLACE_WITH_*|:*|*:) echo 'substitute the node IP and port; not probing'; exit 2 ;;
  esac
  python3 - "$@" <<'PYPROBE'
import ipaddress
import socket
...
try:
    ...
    port = int(sys.argv[2])
    if address.is_unspecified or address.is_loopback or not 1 <= port <= 65535:
        raise ValueError("use the actual non-loopback node IP and a valid port")
...
In an intentionally exposed test deployment, the outside probe should connect. After isolation, the trusted peer must still connect while the outside probe cannot; confirm that firewall logs or counters attribute the latter to the intended rule. A stopped process, changed dynamic port, wrong address, routing failure or local socket error is inconclusive. Capture the handshake while it is listening during startup. This checks TCP reachability only; verify any RDMA or other non-TCP transport separately with the selected backend's tools.
...
- vLLM v0.30.0 parallel configuration defaults and initialization: https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/config/parallel.py#L100-L330 and https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/config/parallel.py#L890-L960; serving address selection: https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/engine/arg_utils.py#L2350-L2500
- vLLM v0.30.0 TCP/file rendezvous and stateless stores: https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/distributed/parallel_state.py#L1793-L1880 and https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/distributed/utils.py#L467-L506; remote message-queue bind: https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/distributed/device_communicators/shm_broadcast.py#L519-L537
- vLLM v0.30.0 single-GPU file-store path and Gloo group creation: https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/v1/executor/uniproc_executor.py#L77-L88 and https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/distributed/parallel_state.py#L494-L508
...
- vLLM v0.30.0 NIXL handshake address and listener: https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/distributed/kv_transfer/kv_connector/v1/nixl/base_scheduler.py#L75-L79 and https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/distributed/kv_transfer/kv_connector/v1/nixl/base_scheduler.py#L345-L382; Mooncake bootstrap and worker listeners: https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/distributed/kv_transfer/kv_connector/v1/mooncake/mooncake_connector.py#L1031-L1035 and https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/distributed/kv_transfer/kv_connector/v1/mooncake/mooncake_connector.py#L1160-L1187
- vLLM v0.30.0 gRPC selector and insecure bind: https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/entrypoints/launchers/cli_args.py#L424-L429 and https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/entrypoints/launchers/grpc_server.py#L87-L118; DP supervisor routes and bind: https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/entrypoints/launchers/dp_supervisor.py#L220-L237 and https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/entrypoints/launchers/dp_supervisor.py#L322-L349
...
- vLLM v0.30.0 API key scope (including `/cohere`), unprotected `/invocations` and profiler routes: https://github.com/vllm-project/vllm/blob/v0.30.0/docs/usage/security.md#L140-L225 and https://github.com/vllm-project/vllm/blob/v0.30.0/vllm/entrypoints/serve/middleware/authenticate.py#L11-L62
