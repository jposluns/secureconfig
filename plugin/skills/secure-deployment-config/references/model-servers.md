...
(
  ...
  { unset -n GIT_DIR GIT_WORK_TREE GIT_CEILING_DIRECTORIES && unset -v GIT_DIR GIT_WORK_TREE GIT_CEILING_DIRECTORIES; } 2>/dev/null ||
    { echo 'cannot clear GIT_DIR, GIT_WORK_TREE or GIT_CEILING_DIRECTORIES in this shell; nothing written'; exit 2; }
  command -v git > /dev/null || { echo 'git not found, so the checkout test cannot run; nothing written'; exit 2; }
  mkdir -p -- "$1"
  chmod 700 -- "$1"
  cd -- "$1"
  if GIT_DISCOVERY_ACROSS_FILESYSTEM=1 git -c safe.directory='*' rev-parse --is-inside-work-tree > /dev/null 2>&1; then
    echo 'that directory is inside a git checkout; choose one outside any checkout; no key written'; exit 2
  fi
...
The block creates `CMD_FLAGS.txt` once and never appends to one. Before it generates a key, it refuses a relative path, a directory inside a git checkout (the checkout's own `user_data` included, found with `git rev-parse --is-inside-work-tree` after clearing the `GIT_*` variables that could redirect that test), and a directory where `CMD_FLAGS.txt` already exists in any form, a dangling symlink included; it refuses to run without `git`, because it could not make the checkout test. A refused directory the block created itself is left behind empty. It makes the directory mode `0700`, `umask 077` creates the file mode `0600`, `set -C` refuses to overwrite an existing file, and it writes nothing unless both keys are 64 hex characters. The keys pass only through the subshell's positional parameters and the builtin `printf`. Add any other flags you want in the file on new lines after the two key lines, and never add or edit a key line: the loader joins the file's lines before splitting them into arguments, and a later `--api-key` or `--admin-key` would silently replace a key. To rotate the keys, run the block again with a new private directory and move your settings across. Start `server.py` from the installation directory with only non-secret values on its command line, substituting the same path inside the quotes:
