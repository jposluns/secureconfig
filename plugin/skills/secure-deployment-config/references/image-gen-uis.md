...
After preparing the environment as the README describes, start it from the pinned checkout. The block refuses to start any other checkout, including one a failed clone left behind, and `./Fooocus` keeps a `CDPATH` in your shell from moving it elsewhere:
