...
intended; a registered end user takes the Authenticated role, which has no content grants until one is added.
...
the same roles and policies as REST. Directus defaults to `HOST=0.0.0.0` on port `8055` and serves the Data
Studio (`/` redirects to `/admin`); on a bare host set `HOST=127.0.0.1`, in Docker publish
...
actually uses. Repeat the paired request with a removed Strapi Content API token, a replaced Directus
`ADMIN_TOKEN`, a copied Hasura admin secret, or an otherwise valid canary JWT signed with a retired signing key
placed in `CMS_NEGATIVE_HEADER`, keeping the valid control, origin, path and body unchanged; each must be
refused. For administrator passwords, use a fresh browser session at the same login origin: the
