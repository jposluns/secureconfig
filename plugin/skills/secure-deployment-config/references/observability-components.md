...
names a tenant. None of them has a login screen or MFA. The Prometheus-family tools and Loki can require
TLS client certificates or basic auth natively, and Jaeger can take an OpenTelemetry authenticator, but
all of these controls are off until you configure them. Keep every listener private
...
that Alertmanager serves `/api/v2` with `Access-Control-Allow-Origin: *`, so a page in an operator's
browser can reach an internal instance.
...
- **HTTP:** use the same `--web.config.file` as above. With it, the loopback run returned `401` to an
  unauthenticated silence creation and `200` to an authenticated status read. Alertmanager's own docs
  call its TLS and basic auth experimental.
- **Gossip:** a single instance does not need it, so set `--cluster.listen-address=` (empty) to stop
  Alertmanager listening for peers. On a loopback run with it empty, Alertmanager held no TCP or UDP socket on 9094. An
  HA pair needs the port reachable by its peers only. Its gossip is plaintext unless you pass
...
- **Behind a proxy:** the security model suggests blocking the mutating paths at the proxy to prevent
  CSRF, and setting CORS headers there for the read-only ones. The README warns not to load-balance
...
That covers OTLP on 4317 and 4318, Jaeger gRPC and Thrift HTTP on 14250 and 14268, Zipkin on 9411,
remote sampling on 5778 and 5779, health on 13133, expvar on 27777, zpages on 27778, and self-metrics on
8888. The docs explain that `JAEGER_LISTEN_HOST` exists for containers, where it is set to `0.0.0.0`.
...
`200`. The v1 `/api/services` returned `404`, because v2.21.0 removed the v1 HTTP endpoints. Probe the
...
`200` with the right credentials. That wiring is inferred from the OpenTelemetry types. Jaeger's
...
- **mTLS on the servers.** The authentication docs describe `http_tls_config` and `grpc_tls_config`
  ...
  header. Plain HTTP got `400`. mTLS does not set `X-Scope-OrgID`, so your agent or proxy still does.
...
listens on every interface for an address with no host. Backlog row 1.108 tracks observing them. On the
host:

```bash
sudo ss -tlnp   # 9100, 9093, 9094, 9091, 16686, 16685, 3100, 9095, 7946: loopback or a private address only
sudo ss -ulnp   # Alertmanager gossip also uses UDP 9094 (dskit memberlist is TCP only)
```
...
pair for node_exporter `/metrics`, Pushgateway pushes, Alertmanager `/api/v2/status` and Jaeger
`/api/v3/services`. For Loki under mTLS, the equivalent is `--cert` and `--key` on an authorized client
...
- Prometheus security model (exporters, Alertmanager, Pushgateway, CORS): https://github.com/prometheus/docs/blob/a0d29881382ad1ea20597d34fc4229984b326576/docs/operating/security.md
