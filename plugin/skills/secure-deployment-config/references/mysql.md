...
Locally, all six shell blocks passed `bash -n` and ShellCheck. Guard-only execution under `bash -u` rejected 76 tested invalid-input cases across the four guarded blocks, including an empty value in every argument position, embedded placeholders, angle brackets, example hostnames, missing/wrong markers, and incorrect argument counts. Each guard also accepted one valid dispatch case. These tests exercised guards only; no database behavior was simulated.
...
### TLS and account requirements: MYSQL-V12
...
For server identity, repeat the positive connection against the same reachable endpoint using a valid but unrelated CA file, or a test hostname resolving to that endpoint but absent from its certificate. Expect certificate verification failure; restore the correct CA and hostname and require success. For the existing `batch` account, use its complete client invocation above and require a successful TLS connection with the intended account. [MySQL certificate verification](https://dev.mysql.com/doc/refman/8.4/en/using-encrypted-connections.html).

### Listeners: MYSQL-V08, listener portion
...
(
  ...
  [ "${1-}" = PASTE_WHOLE_BLOCK ] || { echo "paste the whole block; not connecting"; exit 1; }
  shift
  [ "$#" -eq 1 ] || { echo "supply exactly 1 values; not connecting"; exit 1; }
  case "$1" in
    *REPLACE_WITH_*|*'<'*|*'>'*|*example.com*|"")
      echo "replace every placeholder; not connecting"; exit 1 ;;
    *)
...
If using the role alternative, inspect `app_runtime` and any roles it inherits. In MySQL, expand the named role's effective grants with:

```sql
SHOW GRANTS FOR 'app'@'10.0.0.10' USING app_runtime;
```

In MariaDB, inspect the role directly:

```sql
SHOW GRANTS FOR app_runtime;
```

Also repeat `SHOW GRANTS FOR PUBLIC` on MariaDB 10.11+. A `USAGE ON *.*` line denotes no privileges; it is not equivalent to a global data-access grant. [MySQL SHOW GRANTS](https://dev.mysql.com/doc/refman/8.4/en/show-grants.html), [MariaDB roles](https://mariadb.com/docs/server/security/user-account-management/roles/roles_overview).
...
- MariaDB Password Reuse Check and version notes: https://mariadb.com/docs/server/reference/plugins/password-validation-plugins/password-reuse-check-plugin.md
