...
The endpoint descriptions and sanitization behaviour are documented in the [Actuator reference](https://docs.spring.io/spring-boot/3.5/reference/actuator/endpoints.html#actuator.endpoints.sanitization).
...
Reasoned check: run from outside the trusted management network against the public application origin, then repeat from an authorized network against the intended management URL. Include any configured context or management base path in the substituted URL. Use an administrator identity for the authenticated pass.
...
- Spring Boot 4.1.1 Actuator endpoints and moved EndpointRequest import: https://docs.spring.io/spring-boot/4.1/reference/actuator/endpoints.html
