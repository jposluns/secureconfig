...
**Subscriptions are a long-lived server-to-client stream.** A `subscriptions/listen` POST opens a `text/event-stream` (Server-Sent Events) response that "opens a long-lived notification stream" and "stays open and delivers notifications until the client cancels it". See Subscriptions. A default reverse proxy breaks it two ways: it buffers the response, so notifications arrive late or batched rather than as they happen, and it closes the connection when an idle gap exceeds the read timeout. In the MCP location, disable response buffering and raise the read timeout above the longest expected gap between events:
