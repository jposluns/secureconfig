...
2. **Metadata `issuer` mismatch is rejected (reasoned).** Prerequisite unavailable: a controllable metadata endpoint. Serve two authorization-server metadata documents at the well-known URL built from the issuer `https://as.example.com` (that is, `https://as.example.com/.well-known/oauth-authorization-server`), identical except for `issuer`. Control (accepted): `{"issuer":"https://as.example.com","authorization_endpoint":"https://as.example.com/authorize","token_endpoint":"https://as.example.com/token","response_types_supported":["code"],"code_challenge_methods_supported":["S256"]}`. That is a metadata baseline complete enough for the client to proceed (RFC 8414 section 2 marks `response_types_supported` REQUIRED, and MCP refuses when `code_challenge_methods_supported` is absent), so a rejection is attributable to `issuer` rather than a missing required field. Test (rejected): the same body with only `"issuer":"https://evil.example.net"` changed. Run each from a fresh, cache-cleared pre-discovery state so a cached control document cannot satisfy the test run (RFC 9111 section 4.2), and make the client's other well-known discovery locations, including the OpenID configuration, return a fixed failure in both runs, so any endpoint the client contacts can only have come from the document under test rather than from independently validated metadata. Exposed: the client uses the endpoints from the test document. Fixed: the client accepts the control and rejects the test, attributing the rejection to the `issuer` mismatch and contacting no endpoint obtained from the rejected document. Require evidence that the test document was actually fetched, so a mere absence of endpoint traffic, which a cache hit or a blocked alternative would also produce, is not read as success. Distinguishing source: RFC 8414 section 3.3 ("the data contained in the response MUST NOT be used").
...
4. **No refresh token or long-lived bearer token stored in cleartext (runnable, bounded heuristic).** This is a heuristic scan, not a proof: it searches the client's config and log paths for a refresh-token or bearer-token value in the clear, lists only the filenames (never the secret itself), and separates a clean scan from a scan error. Substitute the client's own path for the placeholder, inside the single quotes on the `set --` line:
   ...
   (
     ...
     if env LC_ALL=C grep -D skip -RlaiE '"refresh_token"[[:space:]]*:[[:space:]]*"[^"]|refresh_token=[[:print:]]+|Bearer[[:space:]]+[ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789._~+/-]{20,}' -- "$1/"; then
...
- RFC 9700 section 3, section 4.2, section 4.2.4, and section 4.4.2, OAuth 2.0 Security Best Current Practice (attacker-accessible stored or logged URLs; Referer leakage and its suppression; single-use `state`; mix-up attacks): https://www.rfc-editor.org/rfc/rfc9700.html#section-4.2.4
...
- RFC 9728 section 3.1, section 3.3, section 7.7, and section 7.10, OAuth 2.0 Protected Resource Metadata (metadata GET request; `resource` validation; SSRF precautions; metadata caching): https://www.rfc-editor.org/rfc/rfc9728.html#section-7.7
...
- OAuth 2.1 draft-ietf-oauth-v2-1-14 section 1.4, section 4.1.2, and section 4.3 (access-token confidentiality; authorization code in the redirect query; refresh-token confidentiality): https://datatracker.ietf.org/doc/html/draft-ietf-oauth-v2-1-14#section-4.3
