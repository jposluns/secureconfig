...
With `--unix_socket`, Flower v2.2.0 sets the socket file's mode to `0777` each time it starts, deleting any socket
left at that path and creating a new one, and it has no option to change the mode. On Linux, connecting to the
socket needs write permission on it and search permission on every directory in its path, so the socket's mode
admits every local account that can reach the directory, and a `chmod` of the socket is undone at the next start.
Put the socket in a directory of its own that only Flower's user and the proxy's group can search (for example
owned by the Flower user, group the proxy's, mode `0750`), check the directory's permissions after each deployment
change, and tighten them if anyone else can search it. Some other systems ignore socket-file permissions entirely,
so the directory is the control there too.
...
# Flower on a unix socket: reasoned, not demonstrated; this authoring environment has no Flower install. The
# directory holding the socket must not grant search (x) to "other", and its group should be the proxy's; the
# socket itself shows 777 by design. Substitute your socket's directory and path.
stat -c '%a %U:%G %n' /run/flower /run/flower/flower.sock
...
- Flower `--unix_socket` sets the socket's mode to `0o777` (pinned tag v2.2.0); Tornado's `bind_unix_socket` removes an existing socket at the path, binds, then applies that mode (pinned tag v6.5.10); on Linux, connecting to a stream socket needs write permission on it, some systems ignore socket permissions, and path lookup needs search permission on each directory: https://github.com/mher/flower/blob/v2.2.0/flower/app.py#L85-L89, https://github.com/tornadoweb/tornado/blob/v6.5.10/tornado/netutil.py#L215-L226, https://man7.org/linux/man-pages/man7/unix.7.html and https://man7.org/linux/man-pages/man7/path_resolution.7.html
