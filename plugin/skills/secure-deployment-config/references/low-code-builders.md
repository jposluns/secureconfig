...
For NocoDB, check the encryption key in the running container rather than in a file: Compose variable
substitution, `env_file:` precedence and YAML values all decide what the process finally sees. The block
reports only whether the key is non-empty and never prints it. Substitute the container name inside the
single quotes.
...
(
  ...
  case "$?" in
    0) echo "NC_CONNECTION_ENCRYPT_KEY is non-empty in the running container" ;;
    3) echo "NC_CONNECTION_ENCRYPT_KEY is EMPTY or unset in the running container: stored database credentials are plaintext" ;;
...
Finally, from a host that should not have access, try a TCP connection to each published port, plus a
port you know is open on that address from this host (for example SSH on 22) as the positive control.
The block uses bash's `/dev/tcp`, so it works for SMTP on 25 as well as the web ports. Substitute the
address (an IP address or host name, without a port or brackets) and the control port inside the single
quotes.

```bash
(
  set -- PASTE_WHOLE_BLOCK 'REPLACE_WITH_THE_SERVICE_ADDRESS' 'REPLACE_WITH_A_KNOWN_OPEN_PORT'
  [ "${1-}" = PASTE_WHOLE_BLOCK ] || { echo "paste the whole block, including its set -- line; not probing"; exit; }
  shift
  [ "$#" -eq 2 ] || { echo "the set -- line needs exactly 2 values; not probing"; exit; }
  case "$1" in *REPLACE_WITH_*|""|*[[:cntrl:]]*|*/*|*\[*|*\]*) echo "substitute a bare address (no brackets, no port) on the set -- line above; not probing"; exit ;; esac
  case "$1" in *:*:*) ;; *:*) echo "give the address without a port; not probing"; exit ;; esac
  ...
  for p in "$2" 80 443 8080 10000 4000 8000 25; do
    # shellcheck disable=SC2016  # the single-quoted script is meant to expand $1 and $2 in the child shell
    err=$(timeout 5 bash -c ': > "/dev/tcp/$1/$2"' probe "$1" "$p" 2>&1)
    case "$?:$err" in
      0:*) echo "$1:$p connected: reachable" ;;
      124:*) echo "$1:$p timed out: filtered, or nothing answered" ;;
      *"Connection refused"*) echo "$1:$p refused" ;;
      *) echo "$1:$p inconclusive, a local or lookup error: ${err:-no message}" ;;
    esac
  done
)
```

Exposed, a port reports "connected". Fixed, it reports "refused" or "timed out"; a firewall that
drops packets produces the timeout. "inconclusive" is a lookup or local error and says nothing about
the port, and if the control port is not "connected", none of the results mean anything. On loopback,
with a test server as the control, the block printed "connected" for the control, "refused" for
closed ports, "timed out" for an unroutable test address, "inconclusive" with the resolver's error for
a mistyped host name, and refused to run for an address with a port, a bracketed address, or a machine
without `timeout`. A "connected" on a port you did not mean to publish is the finding.
