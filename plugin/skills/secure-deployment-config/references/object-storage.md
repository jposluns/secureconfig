...
IAM Access Analyzer for S3 lists buckets whose ACL, bucket policy, or access point policy grants public or cross-account access, but only in a Region that has an account-level external-access analyzer, its findings update asynchronously, and archived findings drop out of the active view; create an analyzer in every Region that holds buckets, review archived as well as active findings, and confirm the Block Public Access settings directly rather than treating an empty list as proof.
...
- Create R2 API tokens with the least permission: `Object Read only` or `Object Read & Write` scoped to specific buckets for applications; `Admin Read & Write` can create and delete buckets and change their configuration (operators only), while `Admin Read only` can inspect buckets and read objects but not create or delete buckets. The secret access key is shown once, so store it in a secret manager ([secrets.md](secrets.md)).
...
- Signed URLs (V4) expire after at most 604800 seconds (7 days); `gcloud storage sign-url --duration=1h` signs with a service account (via `--impersonate-service-account`, or a service-account key you activate, not ordinary `gcloud auth login` user credentials), allowing up to 12 hours with system-managed signing or 7 days with a `--private-key-file`. Public access prevention does not apply to signed URLs, so keep their durations short.
...
# percent-encoded (no " or \ to escape), and the builtin printf keeps it off curl's own argv.
...
- The provider's public-access view is empty: IAM Access Analyzer for S3 (with an analyzer in every bucket Region) shows no active or archived public findings, `gcloud storage buckets describe` shows public access prevention `enforced`, or `inherited` with a `storage.publicAccessPrevention` org policy that enforces it (`inherited` alone is not a pass), the Azure Resource Graph query for `allowBlobPublicAccess` shows `false` on every account, and R2 buckets show no Public Development URL or custom domain.
