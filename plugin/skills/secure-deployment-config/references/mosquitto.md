...
`max_packet_size` disconnects clients that send oversized packets. `message_size_limit` rejects an oversized PUBLISH without forwarding it, while completing the QoS 1/2 acknowledgement flow; it is not a packet-allocation defence. The [packet reader](https://raw.githubusercontent.com/eclipse-mosquitto/mosquitto/v2.0.22/lib/packet_mosq.c) checks the packet cap before allocating the incoming body, whereas the [PUBLISH handler](https://raw.githubusercontent.com/eclipse-mosquitto/mosquitto/v2.0.22/src/handle_publish.c) checks the payload after packet reception.
...
`out` forwards local telemetry to the remote broker; `in` brings remote commands into the local broker. Avoid broad `topic # both` forwarding. On the remote broker, give this dedicated username only the matching permissions:
...
In separate terminals, run this observer block once with `device-01.conf` and once with `device-02.conf`. Wait for successful CONNACK and SUBACK in both before publishing. Keep both running throughout each comparison; repeat the window if either exits.
...
| --- | --- | --- |
| MOSQUITTO-LIVE-1 | OPEN - REASONED; broker configuration acceptance and service behaviour not demonstrated | On pinned 2.0.22 and 2.1.2 deployments, load the applicable configurations and reproduce exposed/fixed authentication, TLS/client-certificate, listener, WebSockets, and actual-message ACL comparisons above. Confirm credential reload and termination of revoked connections. Exercise packet and payload limits separately, queue loss and inflight bounds with slow clients, keepalive negotiation/refusal, per-listener connection caps, and 2.1 global connection/session caps. Where bridges exist, demonstrate trusted TLS and hostname rejection, optional client certificates, scoped inbound/outbound delivery, remote ACL denial, and reconnect backoff. Confirm runtime ownership, readable security files, writable protected state/log paths, and persistence only when enabled. Record versions, commands, diagnostics, matched positive controls, and cleanup without credentials. Configuration inspection alone does not close this row. |
