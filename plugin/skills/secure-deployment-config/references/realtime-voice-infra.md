...
- coturn 4.18.0 `turnutils_uclient` password-buffer trace (`g_upwd` is declared in `uclient.h` line 132 and only read in `uclient.c`, lines 3109 and 3132; `startuclient.c` does not reference it; its only other write is the `-W`-derived password in `mainuclient.c` lines 553-554): https://github.com/coturn/coturn/tree/4.18.0/src/apps/uclient
