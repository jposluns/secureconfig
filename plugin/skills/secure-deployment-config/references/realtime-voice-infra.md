...
turnutils_uclient -v -e REPLACE_WITH_OWNED_PEER_IP REPLACE_WITH_TURN_HOST                                  # no creds: expect no allocation (401)
turnutils_uclient -v -u REPLACE_WITH_USER -w REPLACE_WITH_PASSWORD -e REPLACE_WITH_OWNED_PEER_IP REPLACE_WITH_TURN_HOST  # expect an allocation
```
...
- livekit-cli / `lk` (the `lk token create` command in the Verify rotation test): https://github.com/livekit/livekit-cli
