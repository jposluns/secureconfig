...
status against your image, repeat with a deliberately wrong token, and confirm a correct token returns
the JSON), and the Playwright server on the port you configured (`launchServer` uses an ephemeral port
