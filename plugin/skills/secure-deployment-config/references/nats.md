...
The HTTP monitoring endpoint is off unless configured (the official Docker images' bundled configuration configures it; see section 1's introduction), for example with `http_port: 8222` or the server's `-m 8222` option. `https_port` serves the same data over TLS. It answers `/varz`, `/connz`, `/routez`, and, with JetStream enabled, `/jsz`.
