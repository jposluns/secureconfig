...
(or the per-protocol `PHOENIX_TLS_ENABLED_FOR_HTTP`/`PHOENIX_TLS_ENABLED_FOR_GRPC`) - so its default is TLS
off, not TLS unsupported. Either way terminate TLS deliberately (natively, or at a reverse proxy or tunnel
...
protobuf descriptor (and the CA if TLS is on), so it ships REASONED (backlog row 1.45). Because Phoenix's
gRPC listener always binds `[::]` regardless of `PHOENIX_HOST`, you cannot move it to loopback by host
...
- Arize Phoenix, native TLS for HTTP and gRPC (`PHOENIX_TLS_ENABLED` default `False`, `PHOENIX_TLS_CERT_FILE`, `PHOENIX_TLS_KEY_FILE`, per-protocol toggles; added in 8.29, 2025-04-28): https://arize.com/docs/phoenix/release-notes/04-2025/04-28-2025-tls-support-for-phoenix-server
...
- OpenTelemetry Collector, ancillary listeners (internal metrics `127.0.0.1:8888`; zPages `55679`, pprof `1777`, health-check `13133` diagnostic extensions): https://opentelemetry.io/docs/collector/internal-telemetry/
