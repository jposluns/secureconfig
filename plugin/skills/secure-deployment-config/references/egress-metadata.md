...
#     if [ "$t6_rc" -eq 0 ] && [ -n "$T6" ]; then curl -q -g -s -o /dev/null --noproxy '*' --connect-timeout 5 --max-time 10 -w 'aws6=%{http_code} time_connect=%{time_connect}\n' -H "X-aws-ec2-metadata-token: $T6" 'http://[fd00:ec2::254]/latest/meta-data/'; else echo "no usable IPv6 token (PUT6 exit $t6_rc): inconclusive"; fi
