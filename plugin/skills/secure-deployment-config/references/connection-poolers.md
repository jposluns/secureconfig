...
# then read the relevant settings from the output above: auth_type, auth_hba_file, client_tls_sslmode,
# server_tls_sslmode. And read the effective HBA file:
...
(
  ...
  case "$1|$2" in *REPLACE_WITH_*) echo 'substitute both values; not probing'; exit 2 ;; esac
  # From a client that must NOT bypass the pooler: a direct database connection must be REFUSED by pg_hba.
  psql -X "host=$1 hostaddr=$2 port=5432 dbname=app user=app sslmode=verify-full sslrootcert=/etc/ssl/certs/ca.crt" -c 'SELECT 1;'
)
```

For the backend certificate tests, force and identify a fresh BACKEND connection for each case, establish a trusted-certificate control, then test an untrusted CA, a valid certificate for the wrong host name, and a backend that declines TLS, requiring the specific expected error each time and restoring the control afterward; record pgpool-II's unenforceable upstream-TLS case as a limitation, not a pass. Where no isolated test deployment is available these ship reasoned, with the missing prerequisite and demonstration debt recorded in backlog row 1.79.
