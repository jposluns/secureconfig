...
without one. This document is the bridge: it distills the control patterns that recur across the
...
- **Control:** Bind to loopback (`127.0.0.1`) or a private interface by default. Never expose an
  admin, UI, or data port directly to the internet. Anything public sits behind a TLS reverse
  proxy that carries its own authentication. Reach admin surfaces over SSH forwarding, a tailnet,
  or an identity-aware proxy, not the open internet.
- **Why:** A port bound to `0.0.0.0` on an internet-facing host is found by routine scanning
  within minutes, and direct access to a backend bypasses whatever authentication the proxy adds.
- **Check:** Find the bind-address setting and its current value. Run `ss -tlnp`: any line
  showing `0.0.0.0` or `[::]` for this app is reachable unless a firewall blocks it. From an
  outside network, attempt a connection to each port over IPv4 and IPv6 and confirm it is refused
  or filtered. Confirm the backend cannot be reached directly by IP, skipping the proxy.
...
- **Check:** Run `ss -tlnp` (and `ss -ulnp` for UDP) before and after starting the app, and diff.
  For each new listener, answer: what is it, does it authenticate, and what interface is it bound
  to? Compare against container port publications. Search the app's docs for "metrics", "admin",
  "debug", "cluster", and "replication" ports. Treat any listener you cannot explain as exposed
...
- **Control:** Scope what the app serves or reads to the narrowest directory that works. Exclude
  secret paths (key material, config with credentials, `.git`, home directories, backups). Treat
  shared caches or artifact stores served by URL as public to anyone who can guess the URL unless
  access control is verified.
- **Why:** Apps that serve files, render previews, or proxy paths will happily serve secrets that
  live inside their configured root, and cross-user caches leak one user's content to another.
- **Check:** Enumerate every setting that names a directory the app serves or indexes, and
  confirm no secret material lives under those roots. Request a known-sensitive path through the
  app (a dotfile or config inside the served root) and confirm refusal. Use harmless test files
  to probe traversal and symlink behaviour. As user B, fetch an object created by user A and
  confirm access control applies, not just URL obscurity.
...
- **Check:** Enumerate the non-interactive entry points: APIs, webhook receivers, agent
  registration. Call each without credentials and confirm refusal, then with a valid but
  under-scoped credential and confirm the scope holds. Confirm revocation works. Record the
  deployed version and check the vendor's notes for auth changes affecting these surfaces.

## 10. Container and host posture

- **Control:** Never mount the container runtime socket into a container unless that container is
  explicitly trusted with root on the host: socket access is root-equivalent. Avoid privileged
  mode, broad host mounts, and unneeded capabilities, and run as a non-root user. Filter
  published container ports at the host firewall, and verify the firewall actually applies to
  them.
...
- **Check:** Inspect the container spec for runtime socket mounts (a path ending in a runtime
  socket such as `docker.sock`) and for privileged mode. Confirm the app runs as a non-root UID.
  From an external host, scan the published ports over IPv4 and IPv6 and confirm the firewall
  behaves as intended for each.
...
- **Check:** Reconcile every `ss -tlnp` line against your listener inventory. Probe each surface
  ...
  response content, not status code alone. Repeat the same probe with valid credentials and
  confirm success (the positive control). Record target, version, vantage point, expected result,
  and observed result for each check.
...
the requests directory ([requests/](requests/README.md)) rather than maintaining a private
