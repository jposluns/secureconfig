...
- **Check:** Find the bind-address setting and its current value. Run `ss -tlnp`: any line showing
  ...
  over TLS and enforce whatever authentication that surface requires, the one exception being a
  plain HTTP port whose only response is a redirect to HTTPS. Confirm the backend cannot be reached
  directly by IP, skipping the fronting layer.
