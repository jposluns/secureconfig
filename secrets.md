...
7. **Keys and customer data stay out of AI-model prompts.** A prompt to a public LLM, or to a hosted API outside your data agreement, is a paste, and the leak section below already counts a key that reached "a chat, a log, or a paste" as burned; the fix is not to send it in the first place. Keep API keys, credentials, and customer data out of user prompts, and out of the system prompt and any internal role map placed there: a system prompt is not a secret store, and prompt injection can make a model repeat it ([mcp-servers.md](mcp-servers.md)). An assistant that needs a credential gets a scoped, revocable key through the tool's own configuration, never inline in a prompt.
8. **Unpublished business rules are secrets too.** Proprietary logic such as pricing, allocation, or a customer-specific process is intellectual property, not configuration. Keep it out of public repositories, out of anything that ships to the browser (a `NEXT_PUBLIC_` or `VITE_` value, per [frontend-frameworks.md](frontend-frameworks.md)), and out of a public LLM, on the same reasoning as a key: once it is out, it is out.
