...
| 9090 | InvokeAI; the Prometheus server, which listens on `0.0.0.0:9090` by default with no authentication; and Cockpit, whose socket listens on every address | No authentication by default (Prometheus; InvokeAI single-user mode); Cockpit takes any local account, and refuses root only through `disallowed-users` | [image-gen-uis.md](image-gen-uis.md), [admin-uis.md](admin-uis.md), [server-admin-panels.md](server-admin-panels.md) |
...
| 10000/UDP | Webmin's server discovery socket (`listen=10000`), on every IPv4 address | No credential; it answers discovery requests with the host name and port | [server-admin-panels.md](server-admin-panels.md) |
...
| 20000 | Usermin, Webmin's panel for ordinary users, on every address by default | Unix account passwords | [server-admin-panels.md](server-admin-panels.md) |
| 20000/UDP | Usermin's server discovery socket (`listen=20000`), on every IPv4 address | No credential; it answers discovery requests with the host name and port | [server-admin-panels.md](server-admin-panels.md) |
