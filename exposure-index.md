...
| 27017 | MongoDB; the official Linux images (7.0, 8.0 and 8.3 checked), run as `mongod` with no bind setting, add `--bind_ip_all`, and the Windows images' command includes it (every IPv4 address; IPv6 only with `--ipv6`) | Authorization must be enabled (`authorization: enabled`); on an installation with no existing users or roles, the localhost exception lets a local client create the first administrator; the official Linux images leave authorization off unless both `MONGO_INITDB_ROOT_*` credentials are set or the arguments or `--config` file enable it (one credential alone makes the entrypoint exit), and the Windows images leave it off unless configured | [mongodb.md](mongodb.md), [cloud-firewalls.md](cloud-firewalls.md) |
