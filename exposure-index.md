...
| 7947 | Weaviate's internal cluster API, on every local address | None unless `CLUSTER_BASIC_AUTH_USERNAME` or `CLUSTER_BASIC_AUTH_PASSWORD` is set, then HTTP basic authentication on its data and management handlers (a descriptive `/` route stays open) | [vector-databases.md](vector-databases.md) |
