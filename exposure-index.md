...
| 4317, 4318 | OTLP gRPC and OTLP HTTP receivers, including Jaeger v2's, on localhost in its all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (to `0.0.0.0` in a container, which listens on every interface) | Collector ships with no security until configured; Jaeger: not stated | [llm-observability.md](llm-observability.md), [observability-components.md](observability-components.md) |
...
| 5778, 5779 | Jaeger v2 remote sampling, HTTP and gRPC, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (to `0.0.0.0` in a container, which listens on every interface) | not stated | [observability-components.md](observability-components.md) |
...
| 6831/UDP, 6832/UDP | Jaeger v2 Thrift compact and binary receivers, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (to `0.0.0.0` in a container, which listens on every interface) | not stated | [observability-components.md](observability-components.md) |
...
| 8888 | Jupyter, including RunPod deployments. The Vast.ai Jupyter launch mode uses 8080 instead; also Jaeger v2's self-metrics, on localhost in its all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (to `0.0.0.0` in a container, which listens on every interface) | Jupyter: auth on by default via generated token; Jaeger: not stated | [jupyter.md](jupyter.md), [gpu-clouds.md](gpu-clouds.md), [observability-components.md](observability-components.md) |
...
| 9411 | Jaeger v2 Zipkin receiver, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (to `0.0.0.0` in a container, which listens on every interface) | not stated | [observability-components.md](observability-components.md) |
...
| 13133 | OpenTelemetry Collector health-check extension, when enabled; also Jaeger v2's health check, on localhost in its all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (to `0.0.0.0` in a container, which listens on every interface) | Collector ships with no security until configured; Jaeger: not stated | [llm-observability.md](llm-observability.md), [observability-components.md](observability-components.md) |
| 14250, 14268 | Jaeger v2 gRPC and Thrift HTTP receivers, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (to `0.0.0.0` in a container, which listens on every interface) | not stated | [observability-components.md](observability-components.md) |
...
| 27777, 27778 | Jaeger v2 expvar and zpages, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (to `0.0.0.0` in a container, which listens on every interface) | not stated | [observability-components.md](observability-components.md) |
