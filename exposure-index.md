...
| 19529 | Milvus proxy internal gRPC (v3.0.2 and v2.6.24), on every interface | No credentials checked; `authorizationEnabled` protects the proxy's external client APIs, not this port | [vector-databases.md](vector-databases.md) |
...
| 21123, 21124, 22125, 22222 | Milvus internal gRPC: query node, data node, mix coordinator and streaming node (v3.0.2 and v2.6.24), each on every interface | No credentials checked; internal TLS is one-way and off by default | [vector-databases.md](vector-databases.md) |
