...
| --- | --- | --- | --- |
| 22 | SSH, which should not be publicly reachable | not stated | [cloud-firewalls.md](cloud-firewalls.md), [host.md](host.md), [gpu-clouds.md](gpu-clouds.md) |
| 25 | SMTP relay or MX; the exposure to index is an open relay, not the port itself; also Windmill's email triggers, which its Compose file publishes on 25 through its bundled Caddy | not stated | [transactional-email-posture.md](transactional-email-posture.md), [low-code-builders.md](low-code-builders.md) |
| 53 | Cluster DNS, in the Kubernetes NetworkPolicy egress rules, over both UDP and TCP | not stated | [container-hardening.md](container-hardening.md), [egress-metadata.md](egress-metadata.md) |
| 80, 443 | Usually the TLS proxy, but also native HTTPS listeners in the language guides, and Vaultwarden's container port 80; also Baserow's bundled Caddy, which its Compose file publishes on 0.0.0.0 unless `HOST_PUBLISH_IP` is set, and Windmill's bundled Caddy, which its Compose file publishes on 80 in front of the default login; also Dify's bundled nginx, Onyx's bundled nginx, Text Embeddings Inference's container port 80, PocketBase `serve` with a domain, certbot's standalone HTTP-01 listener, the Flux notification-controller's `webhook-receiver` Service, Argo CD's argocd-server Service, a managed provider's Kubernetes API server, Text Generation Inference's main Docker image, and the Dagster Helm chart's webserver Service (port 80 and type `ClusterIP` by default) | varies by service; see the guides | [nginx.md](nginx.md), [caddy.md](caddy.md), [haproxy.md](haproxy.md), [traefik.md](traefik.md), [apache.md](apache.md), [lighttpd.md](lighttpd.md), [go.md](go.md), [dotnet.md](dotnet.md), [devops-uis.md](devops-uis.md), [low-code-builders.md](low-code-builders.md), [agent-builders.md](agent-builders.md), [ai-infra-services.md](ai-infra-services.md), [cloudflare.md](cloudflare.md), [docker.md](docker.md), [free-certificates.md](free-certificates.md), [gitops-controllers.md](gitops-controllers.md), [host.md](host.md), [java.md](java.md), [kubernetes.md](kubernetes.md), [nodejs.md](nodejs.md), [ollama.md](ollama.md), [pocketbase.md](pocketbase.md), [model-servers.md](model-servers.md), [workflow-orchestrators.md](workflow-orchestrators.md) |
...
| 1883 | MQTT, plaintext (the official Mosquitto images tagged `latest`, `2`, `alpine`, `2.1-alpine` and `2.1.2-alpine`, and the `1.6-openssl` tags, accept anonymous clients on every address as shipped) | Mosquitto 2.0+: no-listener mode anonymous on loopback; explicit listener rejects unauthenticated clients unless `allow_anonymous true` is set, as the official 2.1 images set it | [mosquitto.md](mosquitto.md) |
...
| 3000 | The most crowded port here. The most likely owners are Metabase, Dagster, Gitea, Dokploy, Next.js, SvelteKit's Node adapter (which defaults to `0.0.0.0`), Rails, Flowise, OpenHands, Langfuse, TGI, and the backend behind most proxy examples. Open WebUI's 3000 is a **published host port** mapping to container 8080; and PostgREST, which binds all IPv4 interfaces here by default; and browserless, which is unauthenticated when its TOKEN is unset; also Text Embeddings Inference's CLI default, the Mem0 dashboard, and Onyx's bundled nginx | varies by service; see the guides | [nextjs.md](nextjs.md), [frontend-frameworks.md](frontend-frameworks.md), [ruby.md](ruby.md), [bi-dashboards.md](bi-dashboards.md), [workflow-orchestrators.md](workflow-orchestrators.md), [devops-uis.md](devops-uis.md), [open-webui.md](open-webui.md), [llm-observability.md](llm-observability.md), [model-servers.md](model-servers.md), [agent-builders.md](agent-builders.md), [chat-uis.md](chat-uis.md), [mcp-servers.md](mcp-servers.md), [fronting-auth.md](fronting-auth.md), [headless-cms-instant-api.md](headless-cms-instant-api.md), [headless-browser-services.md](headless-browser-services.md), [ai-infra-services.md](ai-infra-services.md), [caddy.md](caddy.md), [cloud-identity-proxies.md](cloud-identity-proxies.md), [cloudflare.md](cloudflare.md), [docker.md](docker.md), [haproxy.md](haproxy.md), [nginx.md](nginx.md), [tailscale.md](tailscale.md), [traefik.md](traefik.md), [tunnels.md](tunnels.md) |
| 3001 | AnythingLLM, or Uptime Kuma; also Litestream's MCP listener | varies by service; see the guides | [chat-uis.md](chat-uis.md), [devops-uis.md](devops-uis.md), [sqlite.md](sqlite.md) |
...
| 3306 | MySQL and MariaDB (the official images, MySQL 8.4, 9.7 and innovation and MariaDB 10.6 through 13.1 checked, listen on every address unless an argument or option file sets a bind address or `skip_networking`) | Varies by install method; MariaDB 10.4+ local root via Unix socket; on a new data directory the official images create a root account for the root host (`root@'%'` when no value is supplied; none beyond local root when it is `localhost` or a `_FILE` names an empty file), with an empty password when the empty-password variable is set to any non-empty value and no root password, hash or random password is requested | [mysql.md](mysql.md), [cloud-firewalls.md](cloud-firewalls.md) |
...
| 4000 | LiteLLM proxy; also Budibase's LiteLLM publication | At the time of writing, refuses to start without a master key or with `sk-1234`; older or overridden deployments can run without authentication; UI user `admin`, and with `UI_PASSWORD` unset the master key logs in; Budibase's sample `.env` sets the master key to `budibase` | [litellm.md](litellm.md), [low-code-builders.md](low-code-builders.md) |
...
| 4222 | NATS client connections | No authentication configured by default, and the official images' bundled configuration (their default command) adds none | [nats.md](nats.md) |
| 4317, 4318 | OTLP gRPC and OTLP HTTP receivers, including Jaeger v2's, on localhost in its all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (the official image sets `0.0.0.0`, every interface) | Collector ships with no security until configured; Jaeger: not stated | [llm-observability.md](llm-observability.md), [observability-components.md](observability-components.md) |
...
| 5000 | Redash, MLflow tracking server, or a .NET Kestrel default; also text-generation-webui's OpenAI-compatible API | varies by service; see the guides | [bi-dashboards.md](bi-dashboards.md), [mlflow.md](mlflow.md), [dotnet.md](dotnet.md), [model-servers.md](model-servers.md) |
...
| 5432 | PostgreSQL, and pgvector on the same port (the official images, 14 through 19 checked, initialize with `listen_addresses = '*'`, trust local-socket and loopback connections unless `POSTGRES_INITDB_ARGS` sets their methods (`--auth` or `-A`, or `--auth-local` and `--auth-host`), and apply `POSTGRES_HOST_AUTH_METHOD`, a password method by default, to other addresses); the Supabase self-hosted Supavisor pooler also publishes it; also the Onyx development Compose's PostgreSQL and the Langflow example Compose's PostgreSQL, and the Helicone Compose PostgreSQL's container port (published on host port 54388) | varies by service; see the guides | [postgresql.md](postgresql.md), [vector-databases.md](vector-databases.md), [cloud-firewalls.md](cloud-firewalls.md), [supabase-self-hosted.md](supabase-self-hosted.md), [connection-poolers.md](connection-poolers.md), [container-hardening.md](container-hardening.md), [ai-infra-services.md](ai-infra-services.md), [agent-builders.md](agent-builders.md), [llm-observability.md](llm-observability.md) |
...
| 5600 | vLLM NIXL handshake base port; adds the data-parallel index | No authentication on this listener | [model-servers.md](model-servers.md) |
...
| 5671, 5672 | AMQP over TLS, and AMQP plaintext | `guest`/`guest`, usable only from localhost, except in the official Docker image as shipped, which sets `loopback_users.guest = false` | [rabbitmq.md](rabbitmq.md) |
...
| 5778, 5779 | Jaeger v2 remote sampling, HTTP and gRPC, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (the official image sets `0.0.0.0`, every interface) | not stated | [observability-components.md](observability-components.md) |
...
| 6222 | NATS cluster routes, a separate listener with its own authentication and TLS | Run with their default command, the official images' bundled configuration authorizes routes with a user and password published in the image source (`ruser`, `T0pS3cr3t`) | [nats.md](nats.md) |
...
| 6379 | Redis, Valkey, or the Ray head node (the official Redis images checked, 6.2 through 7.4 and 8.10.2, turn protected mode off; the 7.4.8 and 8.10.2 builds, run without a configuration file or arguments, bind every address with no password); also the Onyx development Compose's Redis, and a Helicone Compose backing-service host publication | varies by service; see the guides | [redis.md](redis.md), [ray.md](ray.md), [cloud-firewalls.md](cloud-firewalls.md), [ai-infra-services.md](ai-infra-services.md), [llm-observability.md](llm-observability.md) |
...
| 6831/UDP, 6832/UDP | Jaeger v2 Thrift compact and binary receivers, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (the official image sets `0.0.0.0`, every interface) | not stated | [observability-components.md](observability-components.md) |
...
| 7860 | Gradio, Stable Diffusion WebUI, or Langflow; also text-generation-webui's Gradio UI | varies by service; see the guides | [gradio.md](gradio.md), [image-gen-uis.md](image-gen-uis.md), [agent-builders.md](agent-builders.md), [model-servers.md](model-servers.md) |
| 7865 | Fooocus's Gradio UI, on `127.0.0.1` by default; the upstream container image, Compose file and `docker run` example pass a bare `--listen` (`0.0.0.0`) and publish `7865:7865` with no host address | No authentication unless a valid `auth.json` is present; read from the v2.5.5 source and not observed on a running server, an unreadable file should stop startup, and a malformed file or an empty list should start the UI with no login | [image-gen-uis.md](image-gen-uis.md) |
...
| 8080 | llama.cpp, Weaviate HTTP, Airflow, code-server, Open WebUI's container port, Dify's nginx when mapped to `127.0.0.1:8080`, Spring Boot, Go, and the Vast.ai Jupyter deployment; Keycloak's HTTP port, which exists only when `--http-enabled=true`; and Hasura GraphQL Engine v2, which binds every interface by default; and Argo CD's argocd-server pod listener, fronted by its Service on 80 and 443; and sqlite-web's CLI default HTTP listener, loopback by default (an overridden bind or a published container mapping is the exposure); also NocoDB (listening on every interface by default) and the Appsmith development Compose file's publication; also LocalAI, Text Embeddings Inference's host publication, the Onyx development API, SearxNG's container port, and an `ssh -R` forwarded port on the tunnel server | varies by service; see the guides | [model-servers.md](model-servers.md), [vector-databases.md](vector-databases.md), [workflow-orchestrators.md](workflow-orchestrators.md), [code-server.md](code-server.md), [open-webui.md](open-webui.md), [agent-builders.md](agent-builders.md), [java.md](java.md), [go.md](go.md), [gpu-clouds.md](gpu-clouds.md), [self-hosted-idp.md](self-hosted-idp.md), [headless-cms-instant-api.md](headless-cms-instant-api.md), [gitops-controllers.md](gitops-controllers.md), [sqlite-http-frontends.md](sqlite-http-frontends.md), [low-code-builders.md](low-code-builders.md), [ai-infra-services.md](ai-infra-services.md), [tunnels.md](tunnels.md) |
| 8081 | Hasura's bundled data-connector agent, published beside the engine by the vendor quickstart Compose; also Spring Boot Actuator's management port | not stated | [headless-cms-instant-api.md](headless-cms-instant-api.md), [java.md](java.md) |
...
| 8088 | Apache Superset; also the InfluxDB 1.x backup and restore RPC | Admin password created at first run; change it | [bi-dashboards.md](bi-dashboards.md), [time-series-metrics-stores.md](time-series-metrics-stores.md) |
...
| 8123 | ClickHouse HTTP, plaintext; also the Helicone Compose ClickHouse's container port (published on host port 18123) | ClickHouse base config: `default` user with an empty password, allowed to connect from any address; packaging can change it. Helicone Compose: published independently of the dashboard login; the guide says to rotate the Compose example storage credentials | [clickhouse.md](clickhouse.md), [llm-observability.md](llm-observability.md) |
...
| 8301 | Consul Serf gossip (LAN), on `bind_addr` (every address by default); also Weaviate's internal Raft RPC, on the same address as its Raft | Consul: gossip is unencrypted until an `encrypt` key is set; Weaviate: not stated | [nomad-consul.md](nomad-consul.md), [vector-databases.md](vector-databases.md) |
...
| 8443 | ClickHouse HTTPS, the Kubernetes Dashboard forwarding example, and the configured HTTPS listeners in the Gradio, Python, Java and Ruby guides; Keycloak's HTTPS port; also the Supabase self-hosted Kong override's HTTPS | varies by service; see the guides | [clickhouse.md](clickhouse.md), [devops-uis.md](devops-uis.md), [gradio.md](gradio.md), [python.md](python.md), [java.md](java.md), [ruby.md](ruby.md), [self-hosted-idp.md](self-hosted-idp.md), [supabase-self-hosted.md](supabase-self-hosted.md) |
...
| 8888 | Jupyter, including RunPod deployments. The Vast.ai Jupyter launch mode uses 8080 instead; also Jaeger v2's self-metrics, on localhost in its all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (the official image sets `0.0.0.0`, every interface); also SearxNG, the Mem0 API's host publication, and the OpenTelemetry Collector's internal metrics | Jupyter: auth on by default via generated token; Jaeger: not stated | [jupyter.md](jupyter.md), [gpu-clouds.md](gpu-clouds.md), [observability-components.md](observability-components.md), [ai-infra-services.md](ai-infra-services.md), [llm-observability.md](llm-observability.md) |
| 8998 | vLLM Mooncake producer bootstrap server | No authentication on this listener | [model-servers.md](model-servers.md) |
| 9000 | ClickHouse native TCP (plaintext), MinIO's S3 API, PHP-FPM, TGI's Prometheus listener, or Portainer's legacy HTTP port; Keycloak's management port serving `/health` and `/metrics`, authentik's HTTP port, QuestDB's web console, REST, and SQL endpoints whose Basic-auth keys are unset by default, or the Sentry self-hosted web and API, published here by the bundled nginx; also the Onyx development Compose's inference server, and a Helicone Compose backing-service host publication | varies by service; see the guides | [clickhouse.md](clickhouse.md), [minio.md](minio.md), [php.md](php.md), [model-servers.md](model-servers.md), [devops-uis.md](devops-uis.md), [self-hosted-idp.md](self-hosted-idp.md), [time-series-metrics-stores.md](time-series-metrics-stores.md), [self-hosted-error-trackers.md](self-hosted-error-trackers.md), [ai-infra-services.md](ai-infra-services.md), [llm-observability.md](llm-observability.md) |
| 9001 | MinIO's web console (the `--console-address` / `MINIO_CONSOLE_ADDRESS` port, conventionally 9001); also Mosquitto WebSockets, a Helicone Compose backing-service host publication, and the Onyx development Compose MinIO console's container port (published on host port 9005 by default) | MinIO: `minioadmin`/`minioadmin` when root variables unset | [minio.md](minio.md), [mosquitto.md](mosquitto.md), [llm-observability.md](llm-observability.md), [ai-infra-services.md](ai-infra-services.md) |
...
| 9004, 9005, 9010 | ClickHouse MySQL compatibility, PostgreSQL compatibility, and interserver replica traffic over HTTPS (the HTTP interserver port 9009 is listed separately); also the Onyx development Compose's MinIO publications on 9004 and 9005 | ClickHouse SQL (9004, 9005): base config `default` user with an empty password, allowed to connect from any address; ClickHouse replication (9010): unauthenticated unless `interserver_http_credentials` is set, and independent of SQL users; packaging can change these | [clickhouse.md](clickhouse.md), [ai-infra-services.md](ai-infra-services.md) |
...
| 9090 | InvokeAI; the Prometheus server, which listens on `0.0.0.0:9090` by default with no authentication; and Cockpit, whose socket listens on every address; also Langfuse's bundled MinIO S3 API and Litestream's metrics listener | No authentication by default (Prometheus; InvokeAI single-user mode); Cockpit takes any local account, and refuses root only through `disallowed-users` | [image-gen-uis.md](image-gen-uis.md), [admin-uis.md](admin-uis.md), [server-admin-panels.md](server-admin-panels.md), [llm-observability.md](llm-observability.md), [sqlite.md](sqlite.md) |
...
| 9096 | Grafana Loki gRPC in the vendor's sample `loki-local-config.yaml`; the binary default is 9095 | No built-in authentication | [observability-components.md](observability-components.md) |
...
| 9200 to 9300 | Elasticsearch HTTP default range, binding the first free port; OpenSearch HTTP on 9200, including the Onyx development Compose's OpenSearch | varies by service; see the guides | [elasticsearch.md](elasticsearch.md), [ai-infra-services.md](ai-infra-services.md) |
...
| 9256 | vLLM optional multi-port DP health supervisor | No authentication on this listener | [model-servers.md](model-servers.md) |
...
| 9300 to 9400 | Elasticsearch transport default range (the official images set `network.host: 0.0.0.0`, so unless that is overridden transport listens on all interfaces); authentik's unauthenticated Prometheus metrics on 9300 | varies by service; see the guides | [elasticsearch.md](elasticsearch.md), [self-hosted-idp.md](self-hosted-idp.md) |
| 9411 | Jaeger v2 Zipkin receiver, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (the official image sets `0.0.0.0`, every interface) | not stated | [observability-components.md](observability-components.md) |
...
| 9883 | Mosquitto 2.1's HTTP API and web dashboard (`protocol http_api`), which the official 2.1 images enable on every address (it is not in the image's `EXPOSE`, so `-P` does not publish it) | The official 2.1 images allow it without credentials (`allow_anonymous true`, no password file or ACL); the 2.1.2 manual recommends binding it to loopback behind an authenticating reverse proxy | [mosquitto.md](mosquitto.md) |
...
| 10000 | Budibase's nginx proxy (`MAIN_PORT` in the sample `.env`), which forwards `/db/` to CouchDB; and Webmin's miniserv (HTTPS), on every address by default | Sample `.env` sets `JWT_SECRET` to `testsecret` and CouchDB `budibase`/`budibase`; admin init is open until the first user exists. Webmin: the host's root password logs in with every module | [low-code-builders.md](low-code-builders.md), [server-admin-panels.md](server-admin-panels.md) |
| 10000/UDP | Webmin's server discovery socket (`listen=10000`), on every IPv4 address | not stated; the guide says it lets other Webmin servers find this one | [server-admin-panels.md](server-admin-panels.md) |
...
| 13133 | OpenTelemetry Collector health-check extension, when enabled; also Jaeger v2's health check, on localhost in its all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (the official image sets `0.0.0.0`, every interface) | Collector ships with no security until configured; Jaeger: not stated | [llm-observability.md](llm-observability.md), [observability-components.md](observability-components.md) |
| 14250, 14268 | Jaeger v2 gRPC and Thrift HTTP receivers, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (the official image sets `0.0.0.0`, every interface) | not stated | [observability-components.md](observability-components.md) |
| 15671 | RabbitMQ management UI over TLS, as noted in the guide's listener inventory | `guest`/`guest`, usable only from localhost, except in the official Docker image as shipped, which sets `loopback_users.guest = false` | [rabbitmq.md](rabbitmq.md) |
| 15672 | RabbitMQ management UI | `guest`/`guest`, usable only from localhost, except in the official Docker image as shipped, which sets `loopback_users.guest = false` | [rabbitmq.md](rabbitmq.md) |
...
| 19529 | Milvus proxy internal gRPC (v3.0.2 and v2.6.24), on every interface | No credentials checked; `authorizationEnabled` protects the proxy's external client APIs, not this port; internal TLS is one-way, off by default, and falls back to plaintext if its certificate or key does not load | [vector-databases.md](vector-databases.md) |
...
| 20000 | Usermin, Webmin's panel for ordinary users, on every address by default | not stated | [server-admin-panels.md](server-admin-panels.md) |
| 20000/UDP | Usermin's server discovery socket (`listen=20000`), on every IPv4 address | not stated; the guide says it lets other Webmin servers find this one | [server-admin-panels.md](server-admin-panels.md) |
...
| 21123, 21124, 22125, 22222 | Milvus internal gRPC: query node, data node, mix coordinator and streaming node (v3.0.2 and v2.6.24), each on every interface (the query node, data node and streaming node fall back to a random port, still on every interface, when theirs is taken) | No credentials checked; internal TLS is one-way, off by default, and falls back to plaintext if its certificate or key does not load | [vector-databases.md](vector-databases.md) |
| 25672 | RabbitMQ inter-node and CLI Erlang distribution (by default `RABBITMQ_NODE_PORT` plus 20000; a non-empty `RABBITMQ_DIST_PORT` or `DIST_PORT`, or `distribution.listener.port_range`, can replace it); by default the Erlang cookie is its only credential and grants full control of the node | The Erlang cookie, a shared secret; cookie plus reachability can give full broker control | [rabbitmq.md](rabbitmq.md) |
...
| 27017 | MongoDB; the official Linux images (7.0, 8.0 and 8.3 checked), run as `mongod` with no bind setting, add `--bind_ip_all`, and the Windows images' command includes it (every IPv4 address; IPv6 too only when `net.ipv6` is true) | Authorization must be enabled (`authorization: enabled`); on an installation with no existing users or roles, the localhost exception lets a local client create the first administrator; the official Linux images leave authorization off unless both `MONGO_INITDB_ROOT_*` credentials are set or the arguments or a configuration file (`--config` or `-f`) enable it (one credential alone makes the entrypoint exit), and the Windows images leave it off unless configured | [mongodb.md](mongodb.md), [cloud-firewalls.md](cloud-firewalls.md) |
| 27777, 27778 | Jaeger v2 expvar and zpages, on localhost in the all-in-one configuration unless `JAEGER_LISTEN_HOST` is set (the official image sets `0.0.0.0`, every interface) | not stated | [observability-components.md](observability-components.md) |
| 29501 | vLLM multi-node multiprocessing rendezvous | No authentication on this listener | [model-servers.md](model-servers.md) |
| 29550 | vLLM remote data-parallel startup handshake | No authentication on this listener | [model-servers.md](model-servers.md) |
...
| 33060 | MySQL X Protocol, a separate listener whose bind is not controlled by bind_address; MariaDB does not implement it; the official MySQL images leave it on every address unless configured | not stated | [mysql.md](mysql.md) |
