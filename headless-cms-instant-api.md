...
template Strapi generates sets `HOST` to `env('HOST', '0.0.0.0')`, so a container started from it
binds every interface; bring the instance up on loopback for the bootstrap step (set `HOST` to
`127.0.0.1`) or reach it through a tunnel, and only then expose it.
...
server configuration documents `APP_KEYS` (the session signing keys), and the admin-panel and
API-token configuration add `ADMIN_JWT_SECRET`, `JWT_SECRET`, `API_TOKEN_SALT` and
`TRANSFER_TOKEN_SALT`. A copied example `.env` lets anyone holding the same values forge sessions or
...
and the secret grants "full admin rights", so it is a bearer credential that must never reach a
browser or a client application ([secrets.md](secrets.md)). The engine binds to every interface by
...
role's privileges, so a broad anonymous role also broadens what an unauthenticated caller can
enumerate. Set `jwt-secret` (`PGRST_JWT_SECRET`) to a cryptographically random value of at least 32
...
the optional admin server (`admin-server-port`) binds to that same host, so account for it too.
...
single canary. Read the body, not the bare status: Hasura's GraphQL API answers HTTP 200 even for an
authorization error, so the fixed state is the admin-secret-required message in the JSON, not a 401
(this status behaviour is observed rather than vendor-documented; confirm it against your version).
