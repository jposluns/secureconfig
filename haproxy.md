...
The stats page is off unless a frontend enables it, and enabling it authenticates nothing on its own: an unauthenticated page leaks operational detail, and `stats admin` additionally lets a caller enable, disable, or drain backends. Omit `stats admin` from a monitoring page, keep any admin page private, and reuse the hashed userlist rather than a cleartext `stats auth`:
...
Verify each management surface you enabled, reasoned the same way (backlog row 1.80). For a stats listener,
send an unauthenticated request (must be refused) and one with valid credentials (must return the HAProxy
statistics), against that listener specifically, since a challenge from another frontend proves nothing about
it. For the runtime API, inspect the socket's ownership and mode and attempt the same socket as an
unauthorized local identity, requiring a permission-denied failure; `show cli level` reporting `admin` is the
authorized level, not proof of access control. If you required client certificates (`verify required`), the
Basic-auth probes above cannot reach the challenge without a certificate: test no certificate and an untrusted
one (TLS rejection) and a trusted one with `--cert`/`--key` (success), keeping server-certificate verification
on and adding the Basic credentials only if that rule is still configured.
