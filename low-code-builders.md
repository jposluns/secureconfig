...
pinned `docker/start.sh` only runs `node docker/main.js`, but the image's build recipe is not in the
pinned source, so that was not verified. The block reports only whether the key is non-empty and never
prints it. Substitute the container name inside the single quotes.
...
(
  ...
  case "$?" in
    ...
    3) echo "NC_CONNECTION_ENCRYPT_KEY is EMPTY or unset in the container's configured environment: stored database credentials are plaintext" ;;
...
address, not a host name, so that a name with both IPv4 and IPv6 addresses cannot hide one behind a
timeout on the other: run it once for each public address of the host. It also takes a port you know
...
(
  ...
  shift
  [ "$#" -eq 2 ] || { echo "the set -- line needs exactly 2 values; not probing"; exit; }
  case "$1" in *REPLACE_WITH_*|""|*[!0-9A-Fa-f.:]*) echo "substitute one IP address (no host name, port or brackets) on the set -- line above; not probing"; exit ;; esac
...
Exposed, a port reports "connected". "refused" and "timed out" show only that this host could not
reach the port: a firewall in front of the service produces either, but so can filtering on the
probing host's own network (many providers block outbound SMTP on 25), and the control proves only
its own port. Treat them as consistent with fixed, and take the `ss` and `docker ps` check on the
host as the authority. "inconclusive" is any other result and says nothing about the port. The block
runs the probe in the C locale so that bash's refusal message is matched in English. On loopback it
printed "connected" for the control and an open port, "timed out" for a port whose accept queue was
full (standing in for a filtered one) and "refused" for a closed port; it stopped when the control
was closed, and refused to run for a host name, an address with a port, a bracketed address, or a
machine without `timeout`. A "connected" on a port you did not mean to publish is the finding.
...
- NocoDB 2026.09.0 container start script: https://github.com/nocodb/nocodb/blob/2026.09.0/packages/nocodb/docker/start.sh
