...
`1.2.3`, for an address with a port, a bracketed address or a leading space, and on a machine without
`timeout`. A value with two or more colons is passed on as IPv6; an invalid one, `dead::beef::`,
failed the name lookup at the control and stopped. A "connected" on a port you did not mean to
