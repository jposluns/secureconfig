...
| Metrics | `GET /v1/sys/metrics`, metrics collector token | Anonymous `403`, collector `200`, on every API listener | Anonymous `200` |
| Profiling | `GET /v1/sys/pprof/`, operator authorized for that profiling path | Anonymous `403`, authorized operator `200`, on every API listener | Anonymous `200` |
...
Prometheus output. A collector's metrics-only policy is not a positive control for profiling;
use the separately authorized operator. These checks follow the
