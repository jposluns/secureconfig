...
placeholders substituted, except where a step says otherwise. Exposed states ran on the same node
with the setting under test changed: the listener's anonymous-access settings set to `true`, a broad
policy, and AppRole roles and a delivery policy without the limits. What that setup cannot show is
marked **REASONED** where it occurs: HA standbys, a second independent audit device, the recipient's
own validation, a wildcard bind (the host forbids binding every interface), `ss` run as root, and real
network paths and firewalls; backlog row 1.114 and VAULT-LIVE-1 below track
...
- **Bind and TLS (the fixed bind demonstrated on one node; a wildcard bind, `ss` as root and the
  firewall REASONED):** read the whole socket table. Confirm `8200` is bound to the intended
...
- **Server state (demonstrated for `501`, `503` and `200`; the standby and removed-node codes
  REASONED):** status should report `initialized: true` and `sealed: false`.
...
revocation the root token's lookup succeeded with policy `root` and `ttl` `0s`.
...
read policy got `200` on the inventory secret: the exposed outcomes.
...
- **Token lifetime, use budget and renewal ceiling (demonstrated):** the login response
  ...
  was refused with `403` at about 43 seconds, while the same schedule without an explicit ceiling kept
  a token working.
