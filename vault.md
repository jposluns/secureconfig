...
with `tls_disable = true` on the listener, and one with the section 3 configuration as written, each
with the policies, roles or delivery policy under test set without their limits. What these runs do
...
read policy got `200` on the inventory secret, and a collector whose policy added `kv/data/*` got `200`
on the payments secret: the exposed outcomes.
...
- **SecretID expiry (demonstrated):** leave a separate SecretID unused for more than two minutes
  ...
  only nominally once a minute. With a 20-second test TTL, a SecretID tried 130 seconds past it was
