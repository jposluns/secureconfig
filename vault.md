...
([fronting-auth.md](fronting-auth.md), [nginx.md](nginx.md), [caddy.md](caddy.md)), and add MFA on the
UI ([mfa.md](mfa.md)); the cluster port must keep Vault's own end-to-end TLS regardless of any proxy.
...
networks (whoever reaches `sys/init` first can initialize it and take the root token). The rest are
policy-gated, so the control there is correct policies, not blanket denial.
...
2.30 tracks demonstrating them. A transport failure or a TLS error is inconclusive, never the fixed
state.
...
vault status -format=json              # initialized true, sealed false; exit code 2 is sealed OR a standby
...
The fixed state is that no TCP connection forms: `time_connect` stays `0.000000` and `err` names a
connection-level failure (a refusal, no route, or a filtered-port timeout). Any HTTP answer, on either
port, is the exposure, since `sys/seal-status` replies unauthenticated when reached; a name-resolution
or local-socket error is inconclusive.
