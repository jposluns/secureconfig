...
Use the complete consumer block separately as `order-consumer` and `analytics-reader`, with their own credentials and TLS fixtures. Publish an `orders.created` marker from `order-svc`.
...
| Same consumer operation with a new name and `max_ack_pending: 1001` | An exposed higher/unbounded policy accepts it; the selected consumer acknowledgement limit rejects it. |
