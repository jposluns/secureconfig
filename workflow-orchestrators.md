...
directory connect (root, and a process with `CAP_DAC_OVERRIDE` or `CAP_DAC_READ_SEARCH`, can reach it regardless).
...
the directory through a volume, permissions are checked against numeric IDs, not names, so the proxy's group ID
must be the directory's group ID. Use a filesystem path, never an abstract socket name (one beginning with a NUL
...
# Flower on a unix socket: reasoned, not demonstrated; this authoring environment has no Flower install. Expect
# the directory to show 750 with Flower's user and the proxy-only group; the socket shows 777 by design. The mode
# bits omit ACLs, so getfacl (from the acl package) should list only user::, group:: and other:: entries, with
# other::---; a named user: or group: entry with x lets that account reach the socket. Recheck after each reboot
# and deployment change. Substitute your socket's directory and path.
...
- Flower `--unix_socket` passes the fixed mode `0o777` (pinned tag v2.2.0); Tornado's `bind_unix_socket` removes an existing socket at a filesystem path, binds, then applies that mode, and binds a name with a leading NUL with no mode (pinned tag v6.5.10); on Linux, connecting to a stream socket needs write permission on it and file permissions have no meaning for abstract sockets, path lookup needs search permission on each directory, and `CAP_DAC_OVERRIDE` and `CAP_DAC_READ_SEARCH` override those checks; `RuntimeDirectoryMode=` defaults to `0755` (systemd v257); files under `/run` must be cleared at the beginning of the boot process (FHS 3.0); ACL entries beyond the mode bits: https://github.com/mher/flower/blob/v2.2.0/flower/app.py#L85-L89, https://github.com/tornadoweb/tornado/blob/v6.5.10/tornado/netutil.py#L215-L228, https://man7.org/linux/man-pages/man7/unix.7.html, https://man7.org/linux/man-pages/man7/path_resolution.7.html, https://github.com/systemd/systemd/blob/v257/man/systemd.exec.xml#L1630-L1640, https://refspecs.linuxfoundation.org/FHS_3.0/fhs/ch03s15.html and https://man7.org/linux/man-pages/man5/acl.5.html
