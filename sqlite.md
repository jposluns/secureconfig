...
LiteFS replicates a SQLite file across a cluster's nodes rather than to object storage directly; its docs note the project is pre-1.0 and recommend regular off-site backups as a separate measure, and that backup destination should get the same bucket-privacy treatment as a Litestream replica. LiteFS also runs an HTTP API for replication and administration, and at v0.5.14 no route on it checks any credential and both the server and LiteFS's own client speak only cleartext HTTP (h2c, HTTP/2 without TLS), so reaching the port is the whole access control. The default `http.addr` is `:20202`, which names no host and so listens on every interface, and the example configuration in the LiteFS repository ships the same host-less value; the address is set only in `litefs.yml` (read from the working directory, the home directory, or `/etc/litefs.yml`, with `${VAR}` environment expansion applied unless `-no-expand-env` is passed), no command-line flag sets it, and an empty `addr` still listens on every interface, on a system-chosen port. Anyone who reaches the port can POST `/import` to create or overwrite a named database, GET `/export` to read one in full (and the replication `/stream` to receive every database), POST `/promote` or `/handoff` to move the primary role, POST `/halt` to take a lock that stalls the primary's writes, and POST `/tx` to apply a transaction file to a database; GET `/info`, `/events` and `/metrics` disclose cluster and database state, and the debug routes, served unconditionally, expose Go's `/debug/pprof/` (`cmdline` returns the process command line; `profile` and `trace` run live profiles) and `/debug/vars` (the command line, memory statistics, and a `store` variable carrying primary status and each database's name, transaction id, and lock states).

So keep the API reachable only by the cluster's own nodes over a private network: set `http.addr` to the node's private address, keep `lease.advertise-url` consistent with it (under a Consul lease an unset `advertise-url` is derived as `http://` plus the node's hostname and the HTTP port, and a static lease uses the configured value as given), and never publish port 20202 through an internet-facing port mapping or proxy. The upstream image bakes in no configuration file and declares no `EXPOSE`, so in a container the bind is every interface of the container's network namespace and the exposure is whatever publication the deployment adds. The optional application proxy (`proxy.addr`, `:8080` in the example configuration, started only when `proxy.target` is set) is a separate listener that forwards requests to the application and needs the application's own TLS and authentication controls.
...
Any HTTP response from the untrusted vantage is a reachable listener (the finding), and for LiteFS a reachable listener is the whole administrative API, since no route on it checks any credential; a timeout, DNS, or TLS error, or a failed authorized control, is inconclusive, so corroborate with the bind address and firewall. For Turso (REASONED: this outbound check needs a Turso database and token, and none was available to the authoring environment), prove the HTTP database rejects anonymous queries. Have a valid database token from the secret store ready to paste. The block prompts for it with input hidden, never exports it, and refuses a control character, so a pasted token cannot inject a second header. This keeps the token out of curl's argv, not out of reach of the account that runs the block:
