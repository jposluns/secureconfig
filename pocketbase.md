...
| PocketBase protected files | **REASONED:** GET a known fixture file at `/api/files/REPLACE_WITH_COLLECTION/REPLACE_WITH_RECORD/REPLACE_WITH_FILENAME`. Before protection, its unsigned URL downloads despite locked record access. After enabling Protected and a restrictive `viewRule`, guest and unrelated-user file-token requests must fail; the permitted user's `pb.files.getToken()` and token-bearing SDK URL must download the same bytes. Use the SDK for signed URLs, or feed a protected curl configuration through stdin. See [download checks](https://raw.githubusercontent.com/pocketbase/pocketbase/v0.40.4/apis/file.go). |
...
| --- | --- | --- |
| SELFHOSTED-BACKEND-LIVE-1 | Demonstrate every REASONED comparison above on PocketBase v0.40.4 and Appwrite 2.2.0, recording binary/image identity, commands, response bodies, denied and allowed identities, disposable-data cleanup, and effective settings. Requires writable service fixtures, a container runtime, controlled accounts and mailboxes, S3 and backup fixtures, ClamAV, function execution, a second-host ingress fixture, and the actual client build. Include all five PocketBase actions, manageRule, files, realtime, bootstrap-token handling, MFA, verification, retained-token behavior, additive Appwrite permissions, server-key scopes, registration, TLS, proxy trust, limits, encrypted recovery, uploads, and executor privacy. | Open; live behavior is reasoned, not demonstrated. |
...
- [Appwrite 2.2.0 API-key authorization, scopes, and abuse exemptions](https://raw.githubusercontent.com/appwrite/appwrite/2.2.0/app/controllers/shared/api.php).
